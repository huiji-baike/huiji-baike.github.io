...
function parseTranslate(data){
	...
	//悬赏，光刃
data=data.replace(/(^|[^A-Za-z])(>Havok Soulwail<)(?=[^A-Za-z]|$)/gi, '$1>霍克 靈嘆<');
data=data.replace(/(^|[^A-Za-z])(>Lev the Condemned<)(?=[^A-Za-z]|$)/gi, '$1>被詛咒的莉芙<');
data=data.replace(/(^|[^A-Za-z])(>Ghial the Bone Dancer<)(?=[^A-Za-z]|$)/gi, '$1>骨之舞者 葛西<');
data=data.replace(/(^|[^A-Za-z])(>Justiciar Marron<)(?=[^A-Za-z]|$)/gi, '$1>判官 馬隆<');
data=data.replace(/(^|[^A-Za-z])(>Murakai, Lady of the Night<)(?=[^A-Za-z]|$)/gi, '$1>夜之女 幕蘭凱<');
data=data.replace(/(^|[^A-Za-z])(>Justiciar Kasandra<)(?=[^A-Za-z]|$)/gi, '$1>判官 卡珊卓拉<');
data=data.replace(/(^|[^A-Za-z])(>Rand Stormweaver<)(?=[^A-Za-z]|$)/gi, '$1>織暴者 硬皮<');
data=data.replace(/(^|[^A-Za-z])(>Vess the Disputant<)(?=[^A-Za-z]|$)/gi, '$1>爭論者 薇絲<');
data=data.replace(/(^|[^A-Za-z])(>Verata(?: the Necromancer)?<)(?=[^A-Za-z]|$)/gi, '$1>死靈法師 芙瑞達<');
data=data.replace(/(^|[^A-Za-z])(>Justiciar Kimii<)(?=[^A-Za-z]|$)/gi, '$1>判官 綺米<');
data=data.replace(/(^|[^A-Za-z])(>Droajam, Mage of the Sands<)(?=[^A-Za-z]|$)/gi, '$1>沙之法師 卓加姆<');
data=data.replace(/(^|[^A-Za-z])(>Zaln the Jaded<)(?=[^A-Za-z]|$)/gi, '$1>精疲力竭的薩恩<');
data=data.replace(/(^|[^A-Za-z])(>Royen Beastkeeper<)(?=[^A-Za-z]|$)/gi, '$1>野獸看守者羅彥<');
data=data.replace(/(^|[^A-Za-z])(>Justiciar Sevaan<)(?=[^A-Za-z]|$)/gi, '$1>判官 席凡<');
data=data.replace(/(^|[^A-Za-z])(>Eldritch Ettin<)(?=[^A-Za-z]|$)/gi, '$1>怪異雙頭巨人<');
data=data.replace(/(^|[^A-Za-z])(>Insatiable Vakar<)(?=[^A-Za-z]|$)/gi, '$1>強欲者瓦卡<');
data=data.replace(/(^|[^A-Za-z])(>Vengeful Aatxe<)(?=[^A-Za-z]|$)/gi, '$1>復仇牛頭怪<');
data=data.replace(/(^|[^A-Za-z])(>Amalek the Unmerciful<)(?=[^A-Za-z]|$)/gi, '$1>殘酷者阿莫列克<');
data=data.replace(/(^|[^A-Za-z])(>Carnak the Hungry<)(?=[^A-Za-z]|$)/gi, '$1>飢餓者卡納克<');
data=data.replace(/(^|[^A-Za-z])(>Fronis Irontoe<)(?=[^A-Za-z]|$)/gi, '$1>鐵趾 弗朗尼<');
data=data.replace(/(^|[^A-Za-z])(>Urgoz<)(?=[^A-Za-z]|$)/gi, '$1>爾果<');
data=data.replace(/(^|[^A-Za-z])(>Valis the Rampant<)(?=[^A-Za-z]|$)/gi, '$1>猖獗的瓦里斯<');
data=data.replace(/(^|[^A-Za-z])(>Fenrir<)(?=[^A-Za-z]|$)/gi, '$1>芬瑞<');
data=data.replace(/(^|[^A-Za-z])(>Cerris<)(?=[^A-Za-z]|$)/gi, '$1>瑟瑞絲<');
data=data.replace(/(^|[^A-Za-z])(>Selvetarm<)(?=[^A-Za-z]|$)/gi, '$1>希維塔姆<');
data=data.replace(/(^|[^A-Za-z])(>Sarnia the Red-Handed<)(?=[^A-Za-z]|$)/gi, '$1>染血之手薩妮亞<');
data=data.replace(/(^|[^A-Za-z])(>Mohby Windbeak<)(?=[^A-Za-z]|$)/gi, '$1>莫比 風喙<');
data=data.replace(/(^|[^A-Za-z])(>Destor the Truth Seeker<)(?=[^A-Za-z]|$)/gi, '$1>真實追尋者戴斯特<');
data=data.replace(/(^|[^A-Za-z])(>Charged Blackness<)(?=[^A-Za-z]|$)/gi, '$1>暗黑幽靈<');
data=data.replace(/(^|[^A-Za-z])(>Selenas the Blunt<)(?=[^A-Za-z]|$)/gi, '$1>直言者瑟列娜絲<');
data=data.replace(/(^|[^A-Za-z])(>Rotscale<)(?=[^A-Za-z]|$)/gi, '$1>惡臭骨龍<');
data=data.replace(/(^|[^A-Za-z])(>Justiciar Amilyn<)(?=[^A-Za-z]|$)/gi, '$1>判官 亞蜜琳<');
data=data.replace(/(^|[^A-Za-z])(>Zoldark the Unholy<)(?=[^A-Za-z]|$)/gi, '$1>不潔 咒暗<');
data=data.replace(/(^|[^A-Za-z])(>Maximilian the Meticulous<)(?=[^A-Za-z]|$)/gi, '$1>細心的馬希米連<');
data=data.replace(/(^|[^A-Za-z])(>Korshek the Immolated<)(?=[^A-Za-z]|$)/gi, '$1>宰殺者 科薛克<');
data=data.replace(/(^|[^A-Za-z])(>Joh the Hostile<)(?=[^A-Za-z]|$)/gi, '$1>敵意 舟<');
data=data.replace(/(^|[^A-Za-z])(>Myish, Lady of the Lake<)(?=[^A-Za-z]|$)/gi, '$1>湖之女 蜜希<');
data=data.replace(/(^|[^A-Za-z])(>Barthimus the Provident<)(?=[^A-Za-z]|$)/gi, '$1>算計者巴希穆斯<');
data=data.replace(/(^|[^A-Za-z])(>Frostmaw the Kinslayer<)(?=[^A-Za-z]|$)/gi, '$1>凍擊 弒族者<');
data=data.replace(/(^|[^A-Za-z])(>Calamitous<)(?=[^A-Za-z]|$)/gi, '$1>卡拉米托斯<');
data=data.replace(/(^|[^A-Za-z])(>Kunvie Firewing<)(?=[^A-Za-z]|$)/gi, '$1>火翼 坤維<');
...
data=data.replace(/(^|[^A-Za-z])(>TPS Regulator Golem<)(?=[^A-Za-z]|$)/gi, '$1>TPS調節高輪<');
data=data.replace(/(^|[^A-Za-z])(>Plague of Destruction<)(?=[^A-Za-z]|$)/gi, '$1>疫之破壞者<');
data=data.replace(/(^|[^A-Za-z])(>The Darkness<)(?=[^A-Za-z]|$)/gi, '$1>暗影<');
data=data.replace(/(^|[^A-Za-z])(>Admiral Kantoh<)(?=[^A-Za-z]|$)/gi, '$1>上將 坎托<');
return data;
}