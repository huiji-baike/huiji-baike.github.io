...
data=data.replace(/(^|[^A-Za-z])(>Arachni<)(?=[^A-Za-z]|$)/gi, '$1>奧拉赫妮<');
data=data.replace(/(^|[^A-Za-z])(>The Four Horsemen<)(?=[^A-Za-z]|$)/gi, '$1>多姆四騎士<');
data=data.replace(/(^|[^A-Za-z])(>Remnant of Antiquities<)(?=[^A-Za-z]|$)/gi, '$1>殘餘的斷片<');
data=data.replace(/(^|[^A-Za-z])(>Arbor Earthcall<)(?=[^A-Za-z]|$)/gi, '$1>大地之喚 亞伯<');
data=data.replace(/(^|[^A-Za-z])(>Prismatic Ooze<)(?=[^A-Za-z]|$)/gi, '$1>多彩烏茲<');
data=data.replace(/(^|[^A-Za-z])(>Lord Khobay<)(?=[^A-Za-z]|$)/gi, '$1>領主庫貝<');
data=data.replace(/(^|[^A-Za-z])(>Jedeh the Mighty<)(?=[^A-Za-z]|$)/gi, '$1>強者 捷地<');
data=data.replace(/(^|[^A-Za-z])(>Ssuns, Blessed of Dwayna<)(?=[^A-Za-z]|$)/gi, '$1>薇娜的祝福 桑斯<');
data=data.replace(/(^|[^A-Za-z])(>Justiciar Thommis<)(?=[^A-Za-z]|$)/gi, '$1>司法官 湯米茲<');
data=data.replace(/(^|[^A-Za-z])(>Pywatt the Swift<)(?=[^A-Za-z]|$)/gi, '$1>迅捷 派維特<');
data=data.replace(/(^|[^A-Za-z])(>Harn and Maxine Coldstone<)(?=[^A-Za-z]|$)/gi, '$1>冰石哈恩/麥辛<');
data=data.replace(/(^|[^A-Za-z])(>Fendi Nin<)(?=[^A-Za-z]|$)/gi, '$1>梵帝寧<');
return data;
}