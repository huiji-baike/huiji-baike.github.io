  ...
  var translate = function(data, kAd = true){

    if (!data){return data;}
    //if (kAd) {data=data.replace(/\d\s*/gi,'');}

    data=data.replace(/Extra Luck Bonus/gi, '[份外运气奖]<br>获以往两倍的幸运及不幸运分；获四倍以往的钥匙和开锁道具掉率');
    data=data.replace(/Elonian Support Bonus/gi, '[支援伊洛那奖]<br>获以往两倍的日戟和光明分');
    data=data.replace(/Zaishen Bounty Bonus/gi, '[战承悬赏奖]<br>战承悬赏任务奖品(铜战承币)翻倍');
    data=data.replace(/Factions Elite Bonus/gi, '[二章精英奖]<br>可从凯宁城进入深渊和尔果');
    data=data.replace(/Northern Support Bonus/gi, '[四章支援奖]<br>获以往两倍的阿苏拉, 戴尔狄摩, 黑檀先锋队, 和诺恩分');

    data=data.replace(/Zaishen Mission Bonus/gi, '[战承主线奖]<br>战承主线任务奖品(铜战承币)翻倍');
    data=data.replace(/Pantheon Bonus/gi, '[神殿奖]<br>可免费进入地下和灾难');
    data=data.replace(/Faction Support Bonus\*?/gi, '[二章支援奖]<br>上交分数时，获以往两倍的库兹柯和勒克森分');
    data=data.replace(/Zaishen Vanquishing Bonus/gi, '[战承清图奖]<br>战承清图任务奖品(铜战承币)翻倍');
    data=data.replace(/Random Arenas Bonus/gi, '[随机竞技奖]<br>随机竞技场内获 以往两倍的巴萨泽和斗士分');

    data=data.replace(/Guild Versus Guild Bonus/gi, '[公会战奖]<br>公会战内获 以往两倍的巴萨泽和冠军分');
    data=data.replace(/Competitive Mission Bonus/gi, '[二章竞技奖]<br>翡翠矿场和杨木要塞内获 以往两倍的巴萨泽和帝国分');
    data=data.replace(/Heroes' Ascent Bonus/gi, '[英雄之路奖]<br>英雄之路内获 以往两倍的巴萨泽和英雄称号分');
    data=data.replace(/Codex Arena Bonus/gi, '[Codex竞技奖]<br>Codex竞技场内获 以往两倍的巴萨泽和Codex称号分');
    data=data.replace(/Alliance Battle Bonus/gi, '[同盟战奖]<br>同盟战内获 以往两倍的巴萨泽和帝国分');


      	data=data.replace(/Zaishen Mission/gi, '战承主线任务');
      	data=data.replace(/Zaishen Bounty/gi, '战承悬赏任务');
      	data=data.replace(/Zaishen Combat/gi, '战承对战任务');
      	data=data.replace(/Zaishen Vanquish/gi, '战承清图任务');
      	data=data.replace(/Shining Blade/gi, '光刃通缉令');
      	data=data.replace(/Vanguard Quest/gi, '黑檀先锋队任务');
      	data=data.replace(/Nicholas Sandford/gi, '毁灭前旅行者');

      	data=data.replace(/Guild Versus Guild/gi, '公会战');
      	data=data.replace(/Alliance Battles/gi, '同盟战');
      	data=data.replace(/Heroes' Ascent/gi, '英雄之路');
      	data=data.replace(/Codex Arena/gi, 'Codex竞技场');
      	data=data.replace(/Fort Aspenwood/gi, '杨木要塞');
      	data=data.replace(/Jade Quarry/gi, '翡翠矿场');
      	data=data.replace(/Random Arena/gi, '随机竞技场');

      	//主线任务
      	data=data.replace(/Abaddon's Mouth/gi, '地狱隘口');
      	data=data.replace(/Divinity Coast/gi, '神圣海岸');
      	data=data.replace(/The Great Northern Wall/gi, '北方长城');
      	data=data.replace(/Chahbek Village/gi, '夏贝克村庄');
      	data=data.replace(/A Time for Heroes/gi, '英雄的光采年代');

      	data=data.replace(/Consulate Docks/gi, '领事馆码头');
      	data=data.replace(/Ring of Fire/gi, '火环群岛');
      	data=data.replace(/The Dragon's Lair/gi, '龙穴');
      	data=data.replace(/D'Alessio Seaboard/gi, '达雷西奥海滨');
      	data=data.replace(/Assault on the Stronghold/gi, '袭击根据地');

      	data=data.replace(/Warband of Brothers/gi, '战争军团之友');
      	data=data.replace(/Borlis Pass/gi, '柏里斯通道');
      	data=data.replace(/Moddok Crevice/gi, '摩多克裂缝');
      	data=data.replace(/Nolani Academy/gi, '若拉尼学院');
      	data=data.replace(/Destruction's Depths/gi, '破坏的深渊');

      	data=data.replace(/Venta Cemetery/gi, '凡特墓地');
      	data=data.replace(/A Gate Too Far/gi, '一道过远的门');
      	data=data.replace(/Finding the Bloodstone/gi, '寻找血石');
      	data=data.replace(/Vizunah Square/gi, '微茹广场');
      	data=data.replace(/G.O.L.E.M./gi, '天才操控而让生活丰富迷人的表现');

      	data=data.replace(/Gate of Madness/gi, '疯狂之门');
      	data=data.replace(/The Elusive Golemancer/gi, '乌啦的实验室');
      	data=data.replace(/Hell's Precipice/gi, '地狱悬崖');
      	data=data.replace(/Ruins of Surmia/gi, '苏米亚废墟');
      	data=data.replace(/Curse of the Nornbear/gi, '诺恩熊的诅咒');

      	data=data.replace(/Gate of Pain/gi, '惩罚之门');
      	data=data.replace(/Blood Washes Blood/gi, '血债血偿');
      	data=data.replace(/Bloodstone Fen/gi, '血石沼泽');
      	data=data.replace(/Abaddon's Gate/gi, '亚霸顿之门');
      	data=data.replace(/The Frost Gate/gi, '寒霜之门');

      	data=data.replace(/Against the Charr/gi, '对抗夏尔');

      	//毁灭前旅者材料
      	data=data.replace(/Unnatural SeedS*?/gi, '古怪的种子');
      	data=data.replace(/Enchanted LodestoneS*?/gi, '附魔磁石');
        data=data.replace(/Singed Gargoyle SkullS*?/gi, '烧焦的石像鬼头颅');
      	data=data.replace(/Gargoyle SkullS*?/gi, '石像鬼头颅');
      	data=data.replace(/Dull CarapaceS*?/gi, '阴暗的甲壳');
      	data=data.replace(/Baked HuskS*?/gi, '烧焦外壳');
      	data=data.replace(/Spider LegS*?/gi, '蜘蛛腿');
      	data=data.replace(/Skeletal LimbS*?/gi, '骷髅手臂');
      	data=data.replace(/Grawl NecklaceS*?/gi, '穴居人项链');
      	data=data.replace(/Worn BeltS*?/gi, '破旧的腰带');

      	//黑檀先锋队任务
      	data=data.replace(/Countess Nadya/gi, '伯爵夫人 娜佳');
      	data=data.replace(/Footman Tate/gi, '仆从 泰特');
      	data=data.replace(/Bandits/gi, '草寇');
      	data=data.replace(/Utini Wupwup/gi, '尤提尼 乌坡乌坡');
      	data=data.replace(/Ascalonian Noble/gi, '阿斯卡隆 贵族');
      	data=data.replace(/Undead/gi, '不死族');
      	data=data.replace(/Blazefiend Griefblade/gi, '火焰恶魔 悲伤刀片');
      	data=data.replace(/Farmer Hamnet/gi, '农民 哈姆奈特');

      	//悬赏，光刃
      	data=data.replace(/Havok Soulwail/gi, '霍克 灵叹');
      	data=data.replace(/Lev the Condemned/gi, '被诅咒的莉芙');
      	data=data.replace(/Ghial the Bone Dancer/gi, '骨之舞者 葛西');
      	data=data.replace(/Justiciar Marron/gi, '判官 马隆');
      	data=data.replace(/Murakai, Lady of the Night/gi, '夜之女 幕兰凯');
      	data=data.replace(/Justiciar Kasandra/gi, '判官 卡珊卓拉');
      	data=data.replace(/Rand Stormweaver/gi, '织暴者 硬皮');
      	data=data.replace(/Vess the Disputant/gi, '争论者 薇丝');
      	data=data.replace(/Verata(?: the Necromancer)?/gi, '死灵法师 芙瑞达');
      	data=data.replace(/Justiciar Kimii/gi, '判官 绮米');
      	data=data.replace(/Droajam, Mage of the Sands/gi, '沙之法师 卓加姆');
      	data=data.replace(/Zaln the Jaded/gi, '精疲力竭的萨恩');
      	data=data.replace(/Royen Beastkeeper/gi, '野兽看守者罗彦');
      	data=data.replace(/Justiciar Sevaan/gi, '判官 席凡');
      	data=data.replace(/Eldritch Ettin/gi, '怪异双头巨人');
      	data=data.replace(/Insatiable Vakar/gi, '强欲者瓦卡');
      	data=data.replace(/Vengeful Aatxe/gi, '复仇牛头怪');
      	data=data.replace(/Amalek the Unmerciful/gi, '残酷者阿莫列克');
      	data=data.replace(/Carnak the Hungry/gi, '饥饿者卡纳克');
      	data=data.replace(/Fronis Irontoe/gi, '铁趾 弗朗尼');
      	data=data.replace(/Urgoz/gi, '尔果');
      	data=data.replace(/Valis the Rampant/gi, '猖獗的瓦里斯');
      	data=data.replace(/Fenrir/gi, '芬瑞');
      	data=data.replace(/Cerris/gi, '瑟瑞丝') ;
      	data=data.replace(/Selvetarm/gi, '希维塔姆');
      	data=data.replace(/Sarnia the Red-Handed/gi, '染血之手萨妮亚');
      	data=data.replace(/Mohby Windbeak/gi, '莫比 风喙');
      	data=data.replace(/Destor the Truth Seeker/gi, '真实追寻者戴斯特');
      	data=data.replace(/Charged Blackness/gi, '暗黑幽灵') ;
      	data=data.replace(/Selenas the Blunt/gi, '直言者瑟列娜丝');
      	data=data.replace(/Rotscale/gi, '恶臭骨龙') ;
      	data=data.replace(/Justiciar Amilyn/gi, '判官 亚蜜琳');
      	data=data.replace(/Zoldark the Unholy/gi, '不洁 咒暗');
      	data=data.replace(/Maximilian the Meticulous/gi, '细心的马希米连');
      	data=data.replace(/Korshek the Immolated/gi, '宰杀者 科薛克');
      	data=data.replace(/Joh the Hostile/gi, '敌意 舟');
      	data=data.replace(/Myish, Lady of the Lake/gi, '湖之女 蜜希');
      	data=data.replace(/Barthimus the Provident/gi, '算计者巴希穆斯');
      	data=data.replace(/Frostmaw the Kinslayer/gi, '冻击 弑族者');
      	data=data.replace(/Calamitous/gi, '卡拉米托斯');
      	data=data.replace(/Kunvie Firewing/gi, '火翼 坤维');
      	data=data.replace(/Greves the Overbearing/gi, '傲慢的葛力斯');
      	data=data.replace(/Z'him Monns/gi, '晶蒙');
      	data=data.replace(/The Greater Darkness/gi, '巨大暗影');
      	data=data.replace(/TPS Regulator Golem/gi, '调节高轮');
      	data=data.replace(/Plague of Destruction/gi, '疫之破坏者');
      	data=data.replace(/The Darknesses/gi, '暗影');
      	data=data.replace(/Admiral Kantoh/gi, '上将 坎托');
      	data=data.replace(/Borrguus Blisterbark/gi, '风吼 柏格斯');
      	data=data.replace(/Forgewight/gi, '炼冶维特') ;
      	data=data.replace(/Baubao Wavewrath/gi, '怒浪 保博');
      	data=data.replace(/Joffs the Mitigator/gi, '缓和者 卓夫');
      	data=data.replace(/Rragar Maneater/gi, '拉喀 食人者');
      	data=data.replace(/Chung, the Attuned/gi, '得道者 村');
      	data=data.replace(/Lord Jadoth/gi, '霸王 贾多司');
      	data=data.replace(/Nulfastu, Earthbound/gi, '地缚 纳法斯图');
      	data=data.replace(/(?:The )?Iron Forgeman/gi, '钢铁巨人');
      	data=data.replace(/Magmus/gi, '麦格默斯');
      	data=data.replace(/Mobrin, Lord of the Marsh/gi, '碎之主 魔兵');
      	data=data.replace(/Duncan the Black/gi, '黑色 唐肯');
      	data=data.replace(/Quansong Spiritspeak/gi, '通灵者 魁嵩');
      	data=data.replace(/The Stygian Underlords/gi, '冥狱地王');
      	data=data.replace(/Fozzy Yeoryios/gi, '冻 耀尔伊欧');
      	data=data.replace(/The Black Beast of Arrgh/gi, '黑色野兽 阿尔古');
      	data=data.replace(/Arachni/gi, '奥拉赫妮');
      	data=data.replace(/The Four Horsemen/gi, '多姆四骑士');
      	data=data.replace(/Remnant of Antiquities/gi, '残余的断片');
      	data=data.replace(/Arbor Earthcall/gi, '大地之唤 亚伯');
      	data=data.replace(/Prismatic Ooze/gi, '多彩乌兹');
      	data=data.replace(/Lord Khobay/gi, '领主库贝');
      	data=data.replace(/Jedeh the Mighty/gi, '强者 捷地');
      	data=data.replace(/Ssuns, Blessed of Dwayna/gi, '薇娜的祝福 桑斯');
      	data=data.replace(/Justiciar Thommis/gi, '司法官 汤米兹');
      	data=data.replace(/Pywatt the Swift/gi, '迅捷 派维特');
      	data=data.replace(/Harn and Maxine Coldstone/gi, '冰石哈恩/麦辛');
      	data=data.replace(/Fendi Nin/gi, '梵帝宁');
      	data=data.replace(/Mungri Magicbox/gi, '魔法盒 牧格里');
      	data=data.replace(/Priest of Menzies/gi, '曼席斯的祭司');
      	data=data.replace(/Ilsundur, Lord of Fire/gi, '火之主 音丧多');
      	data=data.replace(/Kepkhet Marrowfeast/gi, '柯博海特 食髓者');
      	data=data.replace(/Commander Wahli/gi, '指挥官 瓦里');
      	data=data.replace(/Kanaxai/gi, '加奈赛');
      	data=data.replace(/Khabuus/gi, '卡布斯');
      	data=data.replace(/Molotov Rocktail/gi, '石尾 摩洛托夫');
      	data=data.replace(/The Stygian Lords/gi, '烈狱领主');
      	data=data.replace(/Dragon Lich/gi, '巫妖之龙');


  	data=data.replace(/Shing Jea Island/gi, '星岬岛');
  	data=data.replace(/Northern Shiverpeaks/gi, '北席娃山脉');
  	data=data.replace(/Southern Shiverpeaks/gi, '南席娃山脉');
  	data=data.replace(/Far Shiverpeaks/gi, '远北席娃山脉(四章)');
  	data=data.replace(/Ring of Fire Islands/gi, '火环岛');
  	data=data.replace(/maguuma jungle/gi, '梅古玛丛林');

  	data=data.replace(/vabbi/gi, '瓦贝');
  	data=data.replace(/the jade sea/gi, '碧玉海');
  	data=data.replace(/crystal desert/gi, '水晶沙漠');
  	data=data.replace(/tarnished coast/gi, '灰暗海岸');
  	data=data.replace(/the desolation/gi, '硫磺地带');
  	data=data.replace(/charr homelands/gi, '夏尔故乡');
  	data=data.replace(/istan/gi, '艾斯坦');

      //旅行者 相关语句，含材料，城镇，及敌人名称
      data=data.replace(/Ice Tooth CaveS*?/gi, '冰牙洞穴');
      data=data.replace(/Anvil RockS*?/gi, '铁砧石');
      data=data.replace(/Frostfire DryderS*?/gi, '霜火蛛化精灵');
      data=data.replace(/Frostfire FangS*?/gi, '霜火尖牙');
  	data=data.replace(/Boreas SeabedS*? \(explorable area\)/gi, '风神海床 (探索区)');
      data=data.replace(/Boreas SeabedS*?/gi, '风神海床');
      data=data.replace(/Pongmei ValleyS*?/gi, '朋美谷');
      data=data.replace(/Rot Wallow TuskS*?/gi, '腐败兽獠牙');
      data=data.replace(/Rot WallowS*?/gi, '腐败兽');
      data=data.replace(/Elona Reach/gi, '伊洛那流域');
      data=data.replace(/Diviner's Ascent/gi, '预言者之坡');
      data=data.replace(/Sand DrakeS*?/gi, '沙龙兽');
      data=data.replace(/Topaz CrestS*?/gi, '黄宝石颈脊');
      data=data.replace(/Rata Sum/gi, '洛达顶点');
      data=data.replace(/Magus Stones/gi, '玛古斯之石');
      data=data.replace(/LifeweaverS*?/gi, '织命者');
      data=data.replace(/BloodweaverS*?/gi, '织血者');
      data=data.replace(/VenomweaverS*?/gi, '织恨者');
      //data=data.replace(/SpiderS*?/gi, '蜘蛛');
      data=data.replace(/Weaver LegS*?/gi, '编织者的腿');
      data=data.replace(/Yahnur MarketS*?/gi, '雅诺尔市集');
      data=data.replace(/Vehtendi Valley/gi, '巍天帝峡谷');
      data=data.replace(/Storm JacarandaS*?/gi, '暴风荆棘');
      data=data.replace(/Mirage IbogaS*?/gi, '幻象伊波枷');
      data=data.replace(/Enchanted Brambles/gi, '魔法树根');
      data=data.replace(/Whistling ThornbrushS*?/gi, '荆棘之藤');
      data=data.replace(/Sentient SporeS*?/gi, '知觉孢子');
      data=data.replace(/AhojS*?/gi, '亚禾');
      data=data.replace(/Bottle of Vabbian WineS*?/gi, '瓦贝红酒');
      data=data.replace(/Jarimiya the Unmerciful/gi, '残酷 贾米里');
      data=data.replace(/Blacktide Den/gi, '黑潮之穴');
      data=data.replace(/Lahtenda Bog/gi, '洛天帝沼泽');
      data=data.replace(/Mandragor ImpS*?/gi, '曼陀罗恶魔');
      data=data.replace(/Mandragor SlitherS*?/gi, '曼陀罗之藤');
      data=data.replace(/Stoneflesh MandragorS*?/gi, '曼陀罗石根');
      data=data.replace(/Mandragor SwamprootS*?/gi, '曼陀罗根');
      data=data.replace(/Vasburg Armory/gi, '维思柏兵营');
  	data=data.replace(/The Eternal GroveS*? \(explorable area\)/gi, '永恒之林 (探索区)');
      data=data.replace(/The Eternal Grove/gi, '永恒之林');
      data=data.replace(/Skill Hungry GakiS*?/gi, '灵巧的饿鬼');
      data=data.replace(/Pain Hungry GakiS*?/gi, '痛苦的饿鬼');
      data=data.replace(/The Time EaterS*?/gi, '时间吞噬者');
      data=data.replace(/The Scar EaterS*?/gi, '疤痕吞噬者');
      data=data.replace(/Quarrel Falls/gi, '怨言瀑布');
      data=data.replace(/SilverwoodS*?/gi, '银树');
      data=data.replace(/Maguuma WarriorS*?/gi, '梅古玛战士');
      data=data.replace(/Maguuma HunterS*?/gi, '梅古玛猎人');
      data=data.replace(/Maguuma ProtectorS*?/gi, '梅古玛守护者');
      data=data.replace(/Maguuma ManeS*?/gi, '梅古玛鬃毛');
      data=data.replace(/Seeker's PassageS*?/gi, '探索者通道');
      data=data.replace(/Salt FlatsS*?/gi, '盐滩');
      data=data.replace(/The Amnoon OasisS*?/gi, '安奴绿洲');
      data=data.replace(/Prophet's PathS*?/gi, '先知之路');
      data=data.replace(/Jade ScarabS*?/gi, '翡翠圣甲虫');
      data=data.replace(/Jade MandibleS*?/gi, '翡翠下颚骨');
      data=data.replace(/Temple of the AgesS*?/gi, '世纪神殿');
      data=data.replace(/Sunjiang DistrictS*? \(explorable area\)/gi, '孙江行政区 (探索区)');
      data=data.replace(/Sunjiang DistrictS*?/gi, '孙江行政区');
      data=data.replace(/Shenzun TunnelsS*?/gi, '申赞通道');
      data=data.replace(/AfflictedS*?/gi, '被感染的');
      data=data.replace(/Putrid CystS*?/gi, '腐败胞囊');
      data=data.replace(/Temple of the AgesS*?/gi, '世纪神殿');
      data=data.replace(/The Black CurtainS*?/gi, '黑色帷幕');
      data=data.replace(/Kessex PeakS*?/gi, '凯席斯山峰');
      data=data.replace(/Talmark WildernessS*?/gi, '突马克荒地');
      data=data.replace(/Forest MinotaurS*?/gi, '森林牛头怪');
      data=data.replace(/Forest Minotaur HornS*?/gi, '森林牛头怪的角');
      data=data.replace(/The AstralariumS*?/gi, '亚斯特拉利姆');
      data=data.replace(/Zehlon ReachS*?/gi, '列隆流域');
      data=data.replace(/Beknur HarborS*?/gi, '别克诺港');
      data=data.replace(/Issnur IslesS*?/gi, '伊斯诺岛');
      data=data.replace(/Skale BlighterS*?/gi, '黑暗鳞怪');
      data=data.replace(/Frigid SkaleS*?/gi, '寒冰鳞怪');
      data=data.replace(/Ridgeback SkaleS*?/gi, '脊背鳞怪');
      
      data=data.replace(/Chef PanjohS*?/gi, '大厨 潘乔');
      data=data.replace(/Bowl of Skalefin SoupS*?/gi, '鳞怪鳍汤');
      data=data.replace(/Sage LandsS*?/gi, '荒原');
      data=data.replace(/Mamnoon LagoonS*?/gi, '玛奴泻湖');
      data=data.replace(/Henge of DenraviS*?/gi, '丹拉维圣地');
      data=data.replace(/Tangle RootS*?/gi, '纠结之根');
      data=data.replace(/Dry TopS*?/gi, '干燥高地');
      data=data.replace(/Root BehemothS*?/gi, '根巨兽');
      data=data.replace(/Behemoth JawS*?/gi, '巨兽颚');
      data=data.replace(/SifhallaS*?/gi, '袭哈拉');
      data=data.replace(/Jaga MoraineS*?/gi, '亚加摩瑞恩');
      data=data.replace(/Berserking BisonS*?/gi, '海冶克狂战士');
      data=data.replace(/Berserking MinotaurS*?/gi, '牛头怪狂战士');
      data=data.replace(/Berserking WendigoS*?/gi, '狂战士 纹帝哥');
      data=data.replace(/Berserking AurochsS*?/gi, '棘狼狂战士');
      data=data.replace(/Berserker HornS*?/gi, '狂战士的角');
      data=data.replace(/Brauer AcademyS*?/gi, '巴尔学院');
      data=data.replace(/Drazach ThicketS*?/gi, '德瑞扎灌木林');
      data=data.replace(/Tanglewood CopseS*?/gi, '谭格坞树林');
      data=data.replace(/Pongmei ValleyS*?/gi, '朋美谷');
      data=data.replace(/UndergrowthS*?/gi, '矮树丛');
      data=data.replace(/Dragon MossS*?/gi, '龙苔');
      data=data.replace(/Dragon RootS*?/gi, '龙根');
      data=data.replace(/Fishermen's HavenS*?/gi, '渔人避风港');
      data=data.replace(/Stingray StrandS*?/gi, '魟鱼湖滨');
      data=data.replace(/Tears of the FallenS*?/gi, '战死者之泪');
      data=data.replace(/Grand DrakeS*?/gi, '强龙兽');
      data=data.replace(/Sanctum CayS*?/gi, '神圣沙滩');
      data=data.replace(/Lightning DrakeS*?/gi, '闪光龙兽');
      data=data.replace(/Spiked CrestS*?/gi, '尖刺的颈脊');
      data=data.replace(/Imperial SanctumS*?/gi, '帝国圣所');
  	data=data.replace(/Raisu PalaceS*? \(explorable area\)/gi, '莱苏皇宫 (探索区)');
      data=data.replace(/Raisu PalaceS*?/gi, '莱苏皇宫');
      data=data.replace(/Soul StoneS*?/gi, '灵魂石');
      data=data.replace(/Tihark OrchardS*?/gi, '提亚克林地');
      data=data.replace(/Forum HighlandsS*?/gi, '高地广场');
      data=data.replace(/Skree WingS*?/gi, '鸟妖翅膀');
      data=data.replace(/SkreeS*?/gi, '鸟妖');
      data=data.replace(/Serenity TempleS*?/gi, '宁静神殿');
      data=data.replace(/Pockmark FlatsS*?/gi, '麻点平原');
      data=data.replace(/Storm RiderS*?/gi, '暴风驾驭者');
      data=data.replace(/Stormy EyeS*?/gi, '暴风之眼');
      data=data.replace(/Gates of KrytaS*?/gi, '科瑞塔之门');
      data=data.replace(/Scoundrel's RiseS*?/gi, '恶汉山丘');
      data=data.replace(/Griffon's MouthS*?/gi, '狮鹭兽隘口');
      data=data.replace(/Spiritwood PlankS*?/gi, '心灵之板');
      data=data.replace(/Tsumei VillageS*?/gi, '苏梅村');
      data=data.replace(/Panjiang PeninsulaS*?/gi, '班让半岛');
      data=data.replace(/Naga HideS*?/gi, '纳迦皮');
      data=data.replace(/NagaS*?/gi, '纳迦');
      data=data.replace(/SifhallaS*?/gi, '袭哈拉');
      data=data.replace(/Drakkar LakeS*?/gi, '卓卡湖');
      data=data.replace(/Frozen ElementalS*?/gi, '冰元素');
      data=data.replace(/Piles*? of Elemental DustS*?/gi, '元素之土');
      //data=data.replace(/Leviathan PitsS*?/gi, '卑尔根温泉');
      //data=data.replace(/Silent SurfS*?/gi, '尼伯山丘');
      //data=data.replace(/OniS*?/gi, '柯瑞塔北部');
      //data=data.replace(/Keen Oni TalonS*?/gi, '硬瘤');
      data=data.replace(/Leviathan PitsS*?/gi, '利拜亚森矿场');
      data=data.replace(/Silent SurfS*?/gi, '寂静之浪');
      data=data.replace(/Seafarer's RestS*?/gi, '航海者休憩处');
      data=data.replace(/OniS*?/gi, '鬼');
      data=data.replace(/Keen Oni TalonS*?/gi, '鬼爪');
      data=data.replace(/Ice Caves of SorrowS*?/gi, '悲伤冰谷');
      data=data.replace(/IcedomeS*?/gi, '冰顶');
      data=data.replace(/Siege Ice GolemS*?/gi, '攻城冰高仑');
      data=data.replace(/Icy LodestoneS*?/gi, '冰磁石');
      data=data.replace(/Augury RockS*?/gi, '占卜之石');
      data=data.replace(/Skyward ReachS*?/gi, '天际流域');
      data=data.replace(/Destiny's GorgeS*?/gi, '命运峡谷');
      data=data.replace(/Prophet's PathS*?/gi, '探索者通道/先知通道');
      data=data.replace(/Salt FlatsS*?/gi, '盐滩');
      data=data.replace(/Storm KinS*?/gi, '风暴魔');
      data=data.replace(/Shriveled EyeS*?/gi, '干枯的眼睛');
      data=data.replace(/Camp HojanuS*?/gi, '何加努营地');
      data=data.replace(/Barbarous ShoreS*?/gi, '野蛮河岸');
      data=data.replace(/CorsairS*?/gi, '海盗');
      data=data.replace(/Gold DoubloonS*?/gi, '金古币');
      data=data.replace(/Dragon's ThroatS*?/gi, '龙喉');
      data=data.replace(/Shadow's PassageS*?/gi, '阴暗通道');
      data=data.replace(/rare material traderS*?/gi, '稀有材料商');
      data=data.replace(/Luxon factionS*?/gi, '勒克森荣誉值');
      data=data.replace(/Jadeite ShardS*?/gi, '硬玉');
      data=data.replace(/Port SledgeS*?/gi, '雪橇港');
      data=data.replace(/Witman's FollyS*?/gi, '威特曼的怪异建筑');
      data=data.replace(/GrawlS*?/gi, '穴居人');
      data=data.replace(/Grawl CroneS*?/gi, '穴居人巫婆');
      data=data.replace(/Intricate Grawl NecklaceS*?/gi, '精细的穴居人项链');
      data=data.replace(/The Mouth of TormentS*?/gi, '苦痛之地隘口');
      data=data.replace(/Crystal OverlookS*?/gi, '水晶高地');
      data=data.replace(/Ruins of MorahS*?/gi, '摩拉废墟');
      data=data.replace(/Mandragor Sand DevilS*?/gi, '曼陀罗沙恶魔');
      data=data.replace(/Mandragor TerrorS*?/gi, '惊骇曼陀罗');
      data=data.replace(/Ravenous MandragorS*?/gi, '曼陀罗贪婪者');
      data=data.replace(/Luminous StoneS*?/gi, '发亮的石头');
      data=data.replace(/Dzagonur BastionS*?/gi, '萨岗诺棱堡');
      data=data.replace(/Wilderness of BahdzaS*?/gi, '巴萨荒野');
      data=data.replace(/Behemoth GravebaneS*?/gi, '剧毒巨兽');
      data=data.replace(/Scytheclaw BehemothS*?/gi, '镰刀爪巨兽');
      data=data.replace(/Behemoth HideS*?/gi, '巨兽皮革');
      data=data.replace(/Rata SumS*?/gi, '洛达顶点');
      data=data.replace(/Riven EarthS*?/gi, '撕裂大地');
      data=data.replace(/RaptorS*?/gi, '毒瑞克斯');
      data=data.replace(/AngorodonS*?/gi, '安哥罗顿');
      data=data.replace(/Saurian BoneS*?/gi, '蜥蜴骨头');
      data=data.replace(/Sanctum CayS*?/gi, '神圣沙滩');
      data=data.replace(/Stingray StrandS*?/gi, '魟鱼湖滨');
      data=data.replace(/Fishermen's HavenS*?/gi, '渔人避风港');
      data=data.replace(/Talmark WildernessS*?/gi, '突马克荒地');
      data=data.replace(/Inferno ImpS*?|FIRE IMPS*?/gi, '地狱小恶魔');
      data=data.replace(/Glowing HeartS*?/gi, '灼热的心脏');
      data=data.replace(/House zu HeltzerS*?/gi, '凤核议院');
      data=data.replace(/FerndaleS*?/gi, '厥谷');
      data=data.replace(/Rare crafting materialS*?/gi, '稀有材料');
      data=data.replace(/Amber ChunkS*?/gi, '琥珀');
      data=data.replace(/Kurzick BureaucratS*?/gi, '库兹柯理事');
      data=data.replace(/Kodlonu HamletS*?/gi, '克拓奴‧哈姆雷特');
      data=data.replace(/Issnur IslesS*?/gi, '伊斯诺岛');
      data=data.replace(/Irontooth DrakeS*?/gi, '钢牙龙兽');
      data=data.replace(/Rilohn RefugeS*?/gi, '里欧恩难民营');
      data=data.replace(/Steelfang DrakeS*?/gi, '硬甲龙兽');
      data=data.replace(/Chunk of Drake FleshS*?/gi, '大块龙兽肉');
      data=data.replace(/Chef LonbahnS*?/gi, '大厨 萝韩');
      data=data.replace(/Drake KabobS*?/gi, '烤龙兽肉');
      data=data.replace(/Beacon's PerchS*?/gi, '毕肯高地');
      data=data.replace(/Lornar's PassS*?/gi, '洛拿斯通道');
      data=data.replace(/Tomb of the Primeval KingsS*?/gi, '先王之墓');
      data=data.replace(/Banished Dream RiderS*?/gi, '被放逐的梦想骑士');
      data=data.replace(/Phantom ResidueS*?/gi, '幻影残留物');
      //data=data.replace(/Zos Shivros ChannelS*?/gi, '山吉之街');
      data=data.replace(/Nahpui QuarterS*? \(explorable area\)/gi, '纳普区 (探索区)');
      data=data.replace(/Nahpui QuarterS*?/gi, '纳普区');
      data=data.replace(/Essence of DragonS*?/gi, '龙之质体');
      data=data.replace(/Essence of KirinS*?/gi, '麒麟之质体');
      data=data.replace(/Essence of PhoenixS*?/gi, '凤之质体');
      data=data.replace(/Essence of TurtleS*?/gi, '龟之质体');
      data=data.replace(/Celestial EssenceS*?/gi, '天神质体');
      data=data.replace(/The Granite CitadelS*?/gi, '花岗岩堡垒');
      data=data.replace(/Spearhead PeakS*?/gi, '尖枪山');
      data=data.replace(/Ice ImpS*?/gi, '冰小恶魔');
      data=data.replace(/Frigid HeartS*?/gi, '冰冻的心脏');
      data=data.replace(/Thirsty RiverS*?/gi, '干枯河流');
      data=data.replace(/The ScarS*?/gi, '断崖');
      data=data.replace(/Destiny's GorgeS*?/gi, '命运峡谷');
      data=data.replace(/Augury RockS*?/gi, '占卜之石');
      data=data.replace(/Skyward ReachS*?/gi, '天际流域');
      data=data.replace(/HydraS*?/gi, '三头龙');
      data=data.replace(/Dessicated Hydra ClawS*?/gi, '干燥的三头龙爪');
      data=data.replace(/Umbral GrottoS*?/gi, '阴影石穴');
      data=data.replace(/Verdant CascadesS*?/gi, '远野瀑布');
      data=data.replace(/Skelk ReaperS*?/gi, '司怪收割者');
      data=data.replace(/Skelk ScourgerS*?/gi, '司怪严惩者');
      data=data.replace(/Skelk AfflictorS*?/gi, '司怪折磨者');
      data=data.replace(/Skelk ClawS*?/gi, '司怪爪');
      data=data.replace(/Vasburg ArmoryS*?/gi, '维思柏兵营');
      data=data.replace(/Morostav TrailS*?/gi, '摩洛神秘通道');
      data=data.replace(/Durheim ArchivesS*?/gi, '杜汉姆卷藏室');
      data=data.replace(/Fungal WallowS*?/gi, '泥泞兽');
      data=data.replace(/TruffleS*?/gi, '松露');
      data=data.replace(/Kodlonu HamletS*?/gi, '克拓奴 哈姆雷特');
      data=data.replace(/Mehtani KeysS*?/gi, '梅坦尼之钥');
      data=data.replace(/CorsairS*?/gi, '海盗');
      data=data.replace(/Silver Bullion CoinS*?/gi, '银铸币');
      data=data.replace(/Camp RankorS*?/gi, '蓝口营地');
      data=data.replace(/Snake DanceS*?/gi, '蛇舞');
      data=data.replace(/Blessed GriffonS*?/gi, '被祝福的狮鹫兽');
      data=data.replace(/Frosted Griffon WingS*?/gi, '冻结的狮鹫兽翅膀');
      data=data.replace(/Augury RockS*?/gi, '占卜之石');
      data=data.replace(/Prophet's PathS*?/gi, '先知之路');
      data=data.replace(/Minotaur (Crystal Desert)S*?/gi, '牛头怪');
      data=data.replace(/Minotaur HornS*?/gi, '牛头怪角');
      data=data.replace(/Zin Ku CorridorS*?/gi, '辛库走廊');
      data=data.replace(/Tahnnakai TempleS*? \(explorable area\)/gi, '谭纳凯神殿 (探索区)');
  	data=data.replace(/Tahnnakai TempleS*?/gi, '谭纳凯神殿');
      data=data.replace(/Jade BrotherhoodS*?/gi, '翡翠兄弟会');
      data=data.replace(/Jade BraceletS*?/gi, '翡翠手镯');
      data=data.replace(/Zen DaijunS*? \(explorable area\)/gi, '祯台郡 (探索区)');
  	data=data.replace(/Zen DaijunS*?/gi, '祯台郡');
      data=data.replace(/Haiju LagoonS*?/gi, '海幽泻湖');
      data=data.replace(/Crimson Skull Spirit LordS*?/gi, '红颅灵王');
      data=data.replace(/Crimson Skull LongbowS*?/gi, '红颅长弓手');
      data=data.replace(/Crimson Skull MentalistS*?/gi, '红颅心灵使');
      data=data.replace(/Crimson Skull PriestS*?/gi, '红颅祭司');
      data=data.replace(/Gold Crimson Skull CoinS*?/gi, '红颅金币');
      data=data.replace(/Mihanu TownshipS*?/gi, '米哈努小镇');
      data=data.replace(/Holdings of ChokhinS*?/gi, '舟克辛卷藏室');
      data=data.replace(/Bull Trainer GiantS*?/gi, 'Bull Trainer Giant');
      data=data.replace(/Pillaged GoodsS*?/gi, '掠夺的货品');
      data=data.replace(/Lion's ArchS*?/gi, '狮子拱门');
      data=data.replace(/North Kryta ProvinceS*?/gi, '科瑞塔北部');
      data=data.replace(/Caromi Tengu BraveS*?/gi, '卡洛米天狗勇士');
      data=data.replace(/Caromi Tengu WildS*?/gi, '卡洛米天狗野人');
      data=data.replace(/Caromi Tengu ScoutS*?/gi, '卡洛米天狗射手');
      data=data.replace(/Feathered Caromi ScalpS*?/gi, '卡洛米羽毛头皮');
      data=data.replace(/Altrumm RuinsS*?/gi, '奥楚兰废墟');
      data=data.replace(/ArborstoneS*? \(explorable area\)/gi, '亭石 (探索区)');
      data=data.replace(/ArborstoneS*?/gi, '亭石');
      data=data.replace(/Vasburg ArmoryS*?/gi, '维思柏兵营');
      data=data.replace(/Morostav TrailS*?/gi, '摩洛神秘通道');
      data=data.replace(/Stone ReaperS*?/gi, '石之收割者');
      data=data.replace(/Stone RainS*?/gi, '石之雨');
      data=data.replace(/Stone SoulS*?/gi, '石之灵');
      data=data.replace(/Stone CarvingS*?/gi, '石雕品');
      data=data.replace(/Honur HillS*?/gi, '霍奴尔丘陵');
      data=data.replace(/Resplendent MakuunS*?/gi, '奢华之城．莫肯');
      data=data.replace(/Dasha VestibuleS*?/gi, '达沙走廊');
      data=data.replace(/Key of AhdashimS*?/gi, '哈达辛之钥');
      data=data.replace(/The Hidden City of AhdashimS*?/gi, '隐藏之城．哈达辛');
      data=data.replace(/Sapphire DjinnS*?/gi, '蓝宝石巨灵');
      data=data.replace(/Sapphire Djinn EssenceS*?/gi, '蓝宝石巨灵精华');
      data=data.replace(/Henge of DenraviS*?/gi, '丹拉维圣地');
      data=data.replace(/Tangle RootS*?/gi, '纠结之恨');
      data=data.replace(/Jungle TrollS*?/gi, '丛林巨魔');
      data=data.replace(/Jungle Troll TuskS*?/gi, '丛林巨魔獠牙');
      data=data.replace(/Wehhan TerracesS*?/gi, '薇恩平台');
      data=data.replace(/Bahdok CavernsS*?/gi, '巴多克洞穴');
      data=data.replace(/Pogahn PassageS*?/gi, '波甘驿站');
      data=data.replace(/Dejarin EstateS*?/gi, '达贾林庄园');
      data=data.replace(/Cracked MesaS*?/gi, '疯狂梅萨');
      data=data.replace(/Stone Shard CragS*?/gi, '巨大岩石怪');
      data=data.replace(/Sentient LodestoneS*?/gi, '知觉磁石');
      data=data.replace(/Marhan's GrottoS*?/gi, '马翰洞穴');
      data=data.replace(/Ice FloeS*?/gi, '浮冰');
      data=data.replace(/Thunderhead KeepS*?/gi, '雷云要塞');
      data=data.replace(/MursaatS*?/gi, '马赛特');
      data=data.replace(/Mursaat TokenS*?/gi, '马赛特记号');
      data=data.replace(/Eye of the NorthS*?/gi, '极地之眼');
      data=data.replace(/Ice Cliff ChasmsS*?/gi, '冰崖裂口');
      data=data.replace(/Gwen's gardenS*?/gi, '关的庭园');
      data=data.replace(/BattledepthsS*?/gi, '战斗深渊');
      data=data.replace(/Chromatic DrakeS*?/gi, '染色龙兽');
      data=data.replace(/Chromatic ScaleS*?/gi, '染色的麟片');
      data=data.replace(/Augury RockS*?/gi, '占卜之石');
      data=data.replace(/The Arid SeaS*?/gi, '枯竭之海');
      data=data.replace(/Dunes of DespairS*?/gi, '绝望沙丘');
      data=data.replace(/Sand GiantS*?/gi, '沙巨人');
      data=data.replace(/Massive JawboneS*?/gi, '粗大下颚骨');
      data=data.replace(/Leviathan PitsS*?/gi, '利拜亚森矿场');
      data=data.replace(/Gyala HatcheryS*? \(explorable area\)/gi, '盖拉孵化所 (探索区)');
  	data=data.replace(/Gyala HatcheryS*?/gi, '盖拉孵化所');
      data=data.replace(/Leviathan ClawS*?/gi, '利拜亚森之爪');
      data=data.replace(/Leviathan EyeS*?/gi, '利拜亚森之眼');
      data=data.replace(/Leviathan MouthS*?/gi, '利拜亚森之口');
      data=data.replace(/Moon ShellS*?/gi, '月贝');
      data=data.replace(/Frontier GateS*?/gi, '边境关所');
      data=data.replace(/Eastern FrontierS*?/gi, '东方边境');
      data=data.replace(/Carrion DevourerS*?/gi, '腐肉蝎');
      data=data.replace(/Whiptail DevourerS*?/gi, '鞭尾蝎');
      data=data.replace(/Plague DevourerS*?/gi, '瘟疫蝎');
      data=data.replace(/Fetid CarapaceS*?/gi, '恶臭的甲壳');
      data=data.replace(/Beacon's PerchS*?/gi, '毕肯高地');
      data=data.replace(/Deldrimor BowlS*?/gi, '戴尔狄摩盆地');
      data=data.replace(/Shiverpeak WarriorS*?/gi, '席娃山脉战士');
      data=data.replace(/Shiverpeak LongbowS*?/gi, '席娃山脉弓手');
      data=data.replace(/Shiverpeak ProtectorS*?/gi, '席娃山脉守护者');
      data=data.replace(/Shiverpeak ManeS*?/gi, '席娃山脉鬃毛');
      data=data.replace(/The MarketplaceS*?/gi, '市集');
      data=data.replace(/Bukdek BywayS*?/gi, '巴德克小径');
      data=data.replace(/Branche*?s*? of Juni BerrieS*?/gi, '柳树枝');
      data=data.replace(/Sunspear SanctuaryS*?/gi, '日戟避难所');
      data=data.replace(/Marga CoastS*?/gi, '马加海岸');
      data=data.replace(/RonjokS*?/gi, '罗鸠村');
      data=data.replace(/ChunoS*?/gi, '周纳');
      data=data.replace(/Insatiable AppetiteS*?/gi, '贪得无厌的食欲');
      data=data.replace(/TomaS*?/gi, '托玛');
      data=data.replace(/Tihark OrchardS*?/gi, '提亚克林地');
      data=data.replace(/Garden of SeborhinS*?/gi, '希伯欣花园');
      data=data.replace(/Forum HighlandsS*?/gi, '高地广场');

      data=data.replace(/Roaring Ether ClawS*?/gi, '苍穹咆哮者之爪');
      data=data.replace(/Seitung HarborS*?/gi, '青函港');
      data=data.replace(/Zen DaijunS*?/gi, '祯邰郡');
      data=data.replace(/Rolls*? of ParchmentS*?/gi, '羊皮纸卷');
  	data=data.replace(/Kaineng CityS*?/gi, '凯宁城');
      data=data.replace(/Kaineng CenterS*?/gi, '凯宁中心');
      data=data.replace(/Xue YiS*?/gi, '薛易');
      data=data.replace(/Wood PlankS*?/gi, '树木');
      data=data.replace(/Rolls*? of ParchmentS*?/gi, '羊皮纸卷');
      data=data.replace(/Doomlore ShrineS*?/gi, '末日传说神殿');
      data=data.replace(/Dalada UplandsS*?/gi, '达拉达山地');
      data=data.replace(/Charr SeekerS*?/gi, '夏尔追寻者');
      data=data.replace(/Charr BlademasterS*?/gi, '夏尔剑术大师');
      data=data.replace(/Charr ProphetS*?/gi, '夏尔先知');
      data=data.replace(/Charr FlameshielderS*?/gi, '夏尔避燃者');
      data=data.replace(/Superb Charr CarvingS*?/gi, '超级夏尔雕刻品');
      data=data.replace(/OlafsteadS*?/gi, '欧拉夫之地');
      data=data.replace(/Varajar FellsS*?/gi, '维拉戛阵地');
      data=data.replace(/Modniir BerserkerS*?/gi, '莫得米狂战士');
      data=data.replace(/Modniir HunterS*?/gi, '莫得米猎人');
      data=data.replace(/Modniir ManeS*?/gi, '莫得米鬃毛');
      data=data.replace(/Leviathan PitsS*?/gi, '利拜亚森矿场');
      data=data.replace(/Rhea's CraterS*?/gi, '席亚火山口');
      data=data.replace(/Outcast WarriorS*?/gi, '被流放的战士');
      data=data.replace(/Outcast AssassinS*?/gi, '被流放的暗杀者');
      data=data.replace(/Outcast RitualistS*?/gi, '被流放的祭祀者');
      data=data.replace(/Majesty's RestS*?/gi, '王者安息地');
      data=data.replace(/Druid's OverlookS*?/gi, '德鲁伊高地');
      data=data.replace(/Temple of the AgesS*?/gi, '世纪神殿');
      data=data.replace(/Thorn DevourerS*?/gi, '棘刺蝎');
      data=data.replace(/Fevered DevourerS*?/gi, '热病蝎');
      data=data.replace(/Thorny CarapaceS*?/gi, '多刺的甲壳');
      data=data.replace(/Bone PalaceS*?/gi, '白骨宫殿');
      data=data.replace(/The Alkali PanS*?/gi, '金属熔炉');
      data=data.replace(/Ruby DjinnS*?/gi, '红宝石巨灵');
      data=data.replace(/Ruby Djinn EssenceS*?/gi, '红宝石巨灵精华');
      data=data.replace(/Piken SquareS*?/gi, '派肯广场');
      data=data.replace(/The BreachS*?/gi, '缺口');
  	  data=data.replace(/Old AscalonS*?/gi, '旧阿斯卡隆');
  	  data=data.replace(/Ascalon FoothillsS*?/gi, '阿斯卡隆丘陵');
      data=data.replace(/AscalonS*?/gi, '阿斯卡隆');
	  
	  data=data.replace(/Fibrous Mandragor RootS*?/gi, '纤维曼陀罗根');
	  
      data=data.replace(/Charr HideS*?/gi, '夏尔皮');
      data=data.replace(/Ventari's RefugeS*?/gi, '凡特里庇护所');
      data=data.replace(/The FallsS*?/gi, '陷落区');
      data=data.replace(/The Fissure of WoeS*?/gi, '灾难裂痕');
      data=data.replace(/Forest of the Wailing LordS*?/gi, '悲鸣领主区');
      data=data.replace(/Gloom SeedS*?/gi, '黑暗种子');
      data=data.replace(/Breaker HollowS*?/gi, '断崖谷');
      data=data.replace(/Mount QinkaiS*?/gi, '今凯山');
      data=data.replace(/Naga WarriorS*?/gi, '迦纳战士');
      data=data.replace(/Naga ArcherS*?/gi, '迦纳弓手');
      data=data.replace(/Naga RitualistS*?/gi, '迦纳祭祀者');
      data=data.replace(/Naga SkinS*?/gi, '迦纳外皮');
      data=data.replace(/Riverside ProvinceS*?/gi, '河畔地带');
      data=data.replace(/Twin Serpent LakesS*?/gi, '双头蛇湖泊');
      data=data.replace(/Lion's ArchS*?/gi, '狮子拱门');
      
	  data=data.replace(/Bog Skale FinS*?/gi, '泥鳞怪的鳍');
	  
	  data=data.replace(/Skale FinS*?/gi, '鳞怪鳍');
	  
	  data=data.replace(/Bog SkaleS*?/gi, '泥鳞怪');
      data=data.replace(/Twin Serpent LakesS*?/gi, '双头蛇湖泊');
      data=data.replace(/Gruhn the FisherS*?/gi, '渔人古露恩');
      
	  
      data=data.replace(/HerringS*?/gi, '鲱鱼');
      data=data.replace(/Doomlore ShrineS*?/gi, '末日传说神殿');
      data=data.replace(/Sacnoth ValleyS*?/gi, '圣诺谷');
      data=data.replace(/Grawl ChampionS*?/gi, '穴居人冠军');
      data=data.replace(/Grawl Dark PriestS*?/gi, '穴居人黑暗祭司');
      data=data.replace(/Stone Grawl NecklaceS*?/gi, '石穴居人项链');
      data=data.replace(/Jokanur DiggingsS*?/gi, '卓坎诺挖掘点');
      data=data.replace(/Fahranur, The First CityS*?/gi, '旧城 法兰努尔');
      data=data.replace(/Beautiful IbogaS*?/gi, '美丽伊波枷');
      data=data.replace(/Fanged IbogaS*?/gi, '毒牙伊波枷');
      data=data.replace(/Sentient SeedS*?/gi, '知觉种子');
      data=data.replace(/Seitung HarborS*?/gi, '青函港');
      data=data.replace(/Saoshang TrailS*?/gi, '绍商小径');
      data=data.replace(/Mantid DarkwingS*?/gi, '螳螂黑翼');
      data=data.replace(/Mantid GlitterfangS*?/gi, '螳螂锐牙');
      data=data.replace(/Mantid PincerS*?/gi, '螳螂镰');
      data=data.replace(/Ember Light CampS*?/gi, '残火营地');
      data=data.replace(/Perdition RockS*?/gi, '破灭石');
      data=data.replace(/Mahgo HydraS*?/gi, '码果三头龙');
      data=data.replace(/Mahgo ClawS*?/gi, '码果的爪');
      data=data.replace(/Yohlon HavenS*?/gi, '犹朗避难所');
      data=data.replace(/Arkjok WardS*?/gi, '阿尔科监禁区');
      data=data.replace(/Mandragor ImpS*?/gi, '曼陀罗恶魔');
      data=data.replace(/Mandragor SlitherS*?/gi, '曼陀罗撕裂者');
      data=data.replace(/Mandragor RootS*?/gi, '曼陀罗根');
      data=data.replace(/Yohlon HavenS*?/gi, '犹朗避难所');
      data=data.replace(/YajideS*?/gi, '叶吉达');
      data=data.replace(/Mandragor Root CakeS*?/gi, '曼陀罗根糕点');
      data=data.replace(/Mandragor RootS*?/gi, '曼陀罗根');
      data=data.replace(/Snake DanceS*?/gi, '蛇舞');
      data=data.replace(/Dreadnought's DriftS*?/gi, '无惧者之丘');
      data=data.replace(/Beacon's PerchS*?/gi, '毕肯高地');
      data=data.replace(/Deldrimor War CampS*?/gi, '戴尔狄摩兵营');
      data=data.replace(/Azure ShadowS*?/gi, '湛蓝阴影');
      data=data.replace(/Azure RemainS*?/gi, '湛蓝残留物');
      data=data.replace(/The MarketplaceS*?/gi, '市集');
      data=data.replace(/Wajjun BazaarS*?/gi, '瓦江市场');
      data=data.replace(/Am FahS*?/gi, '安费');
      data=data.replace(/Plague IdolS*?/gi, '瘟疫法器');
      data=data.replace(/Tarnished HavenS*?/gi, '灰暗避难所');
      data=data.replace(/Alcazia TangleS*?/gi, '纠结之艾卡滋亚');
      data=data.replace(/Umbral GrottoS*?/gi, '阴影石穴');
      data=data.replace(/Verdant CascadesS*?/gi, '原野瀑布');
      data=data.replace(/Quetzal CrestS*?/gi, '长尾冠毛');
  	  data=data.replace(/QuetzalS*?/gi, '长尾');
      data=data.replace(/The Mouth of TormentS*?/gi, '苦痛之地隘口');
      data=data.replace(/Poisoned OutcropsS*?/gi, '剧毒地表');
      data=data.replace(/MargoniteS*?/gi, '玛骨奈');
      data=data.replace(/Margonite MaskS*?/gi, '玛骨奈面具');
      data=data.replace(/The Granite CitadelS*?/gi, '花岗岩堡垒');
      data=data.replace(/Tasca's DemiseS*?/gi, '塔斯加之死');
      data=data.replace(/Mineral SpringsS*?/gi, '矿物泉源');
      data=data.replace(/TenguS*?/gi, '天狗');
      data=data.replace(/AvicaraS*?/gi, '阿比卡拉');
      data=data.replace(/Feathered Avicara ScalpS*?/gi, '阿比卡拉头皮羽毛');
      data=data.replace(/Fort RanikS*?/gi, '瑞尼克要塞');
      data=data.replace(/Regent ValleyS*?/gi, '统治者之谷');
      data=data.replace(/Red Iris FlowerS*?/gi, '红色鸢尾花');
      data=data.replace(/Grendich CourthouseS*?/gi, '葛兰迪法院');
      data=data.replace(/Flame Temple CorridorS*?/gi, '夏尔火焰神殿');

      data=data.replace(/Charr CarvingS*?/gi, '夏尔雕刻品');
      data=data.replace(/Yak's BendS*?/gi, '牦牛村');
      data=data.replace(/Traveler's ValeS*?/gi, '旅人谷');
      data=data.replace(/Rare material traderS*?/gi, '稀有材料商人');
      data=data.replace(/Bolts*? of LinenS*?/gi, '亚麻布');
      data=data.replace(/ArtisanS*?/gi, '工匠');
      data=data.replace(/Plant FiberS*?/gi, '植物纤维');
      data=data.replace(/Gunnar's HoldS*?/gi, '甘拿的占领地');
      data=data.replace(/Norrhart DomainsS*?/gi, '诺恩之心领地');
      data=data.replace(/Dreamroot MandragorS*?/gi, '梦之根曼陀罗');
      data=data.replace(/Mandragor ScavengerS*?/gi, '拾荒曼陀罗');
      data=data.replace(/Mystic MandragorS*?/gi, '秘教曼陀罗');
      data=data.replace(/Ulcerous MandragorS*?/gi, '已腐蚀曼陀罗');
      data=data.replace(/Frigid Mandragor HuskS*?/gi, '呆板曼陀罗外壳');
      data=data.replace(/Champion's DawnS*?/gi, '勇士曙光');
      data=data.replace(/Cliffs of DohjokS*?/gi, '杜夏悬崖');
      data=data.replace(/CorsairS*?/gi, '海盗');
      data=data.replace(/Copper ShillingS*?/gi, '铜先令');
      data=data.replace(/The WildsS*?/gi, '荒原');
      data=data.replace(/Sage LandsS*?/gi, '贤者领地');
      data=data.replace(/Wind Rider (Maguuma Jungle)S*?/gi, '驭风者');
      data=data.replace(/Ancient EyeS*?/gi, '远古之眼');
      data=data.replace(/Sunspear SanctuaryS*?/gi, '日戟避难所');
      data=data.replace(/Sunward MarchesS*?/gi, '向阳边境');
      data=data.replace(/Mirage IbogaS*?/gi, '幻象伊波枷');
      data=data.replace(/Murmuring ThornbrushS*?/gi, '荆棘之藤');
      data=data.replace(/Sentient SporeS*?/gi, '知觉孢子');
      data=data.replace(/Gates of KrytaS*?/gi, '科瑞塔关所');
      data=data.replace(/Scoundrel's RiseS*?/gi, '恶汉山丘');
      data=data.replace(/Bog Skale FinS*?/gi, '泥鳞怪的鳍');
      data=data.replace(/Bog SkaleS*?/gi, '泥鳞怪');
      data=data.replace(/Zos Shivros ChannelS*?/gi, '佐席洛斯水道');
      data=data.replace(/Kraken SpawnS*?/gi, '海妖卵');
      data=data.replace(/Kraken EyeS*?/gi, '海妖之眼');
      data=data.replace(/Grendich CourthouseS*?/gi, '葛兰迪法院');
      data=data.replace(/Flame Temple CorridorS*?/gi, '夏尔火焰神殿');
      data=data.replace(/Dragon's GulletS*?/gi, '巨龙峡谷');
      data=data.replace(/Abomination (NPC)S*?/gi, '憎恨者');
      data=data.replace(/Gruesome RibcageS*?/gi, '可怕的胸腔');
      data=data.replace(/Longeye's LedgeS*?/gi, '长眼岩脉');
      data=data.replace(/Grothmar WardownsS*?/gi, '古斯玛战争丘陵地');
      data=data.replace(/Mandragor Dust DevilS*?/gi, '曼陀罗尘魔');
      data=data.replace(/Mandragor Smoke DevilS*?/gi, '曼陀罗烟魔');
      data=data.replace(/Vile MandragorS*?/gi, '暗黑曼陀罗');
      
      data=data.replace(/Chantry of SecretsS*?/gi, '隐密教堂');
      data=data.replace(/Yatendi CanyonsS*?/gi, '亚天帝峡谷');
      data=data.replace(/Rain BeetleS*?/gi, '雨甲虫');
      data=data.replace(/Rock BeetleS*?/gi, '石甲虫');
      data=data.replace(/GeodeS*?/gi, '晶石');
      data=data.replace(/Vizunah Square (Local Quarter)S*?/gi, '薇茹广场本地');
      data=data.replace(/The UndercityS*?/gi, '地下城');
      data=data.replace(/Kappa (level 20)S*?/gi, '河童');
      data=data.replace(/Ancient Kappa ShellS*?/gi, '古河童壳');
      data=data.replace(/Temple of the AgesS*?/gi, '世纪神殿');
      data=data.replace(/The Black CurtainS*?/gi, '黑色帷幕');
      data=data.replace(/Fog NightmareS*?/gi, '迷雾梦靥');
      data=data.replace(/Shadowy RemnantsS*?/gi, '阴影残留物');
      data=data.replace(/Remains of SahlahjaS*?/gi, '萨拉迦遗址');
      data=data.replace(/The Sulfurous WastesS*?/gi, '硫磺荒地');
      data=data.replace(/Awakened CavalierS*?/gi, '觉醒的骑士');
      data=data.replace(/Mummy WrappingS*?/gi, '木乃伊裹尸布');
      data=data.replace(/Tsumei VillageS*?/gi, '苏梅村');
      data=data.replace(/Sunqua ValeS*?/gi, '桑泉谷');
      data=data.replace(/SensaliS*?/gi, '圣沙利天狗');
      data=data.replace(/Feathered ScalpS*?/gi, '羽头皮');
      data=data.replace(/Bone PalaceS*?/gi, '白骨宫殿');
      data=data.replace(/Joko's DomainS*?/gi, '杰格领地');
      data=data.replace(/Gate of TormentS*?/gi, '苦痛之门');
      data=data.replace(/Nightfallen JahaiS*?/gi, '夜蚀暗殒 夏亥');
      data=data.replace(/Graven MonolithS*?/gi, '铭刻石雕');
      data=data.replace(/Inscribed ShardS*?/gi, '铭刻碎片');
      data=data.replace(/Vlox's FallsS*?/gi, '弗洛克斯瀑布');
      data=data.replace(/Arbor BayS*?/gi, '亚伯湾');
      data=data.replace(/Krait SkinS*?/gi, '环蛇皮');
      data=data.replace(/KraitS*?/gi, '环蛇');
      data=data.replace(/The Granite CitadelS*?/gi, '花岗岩堡垒');
      data=data.replace(/Tasca's DemiseS*?/gi, '塔斯加之死');
      data=data.replace(/Stone Summit BadgeS*?/gi, '石峰标志');
      data=data.replace(/Stone SummitS*?/gi, '石峰矮人');
      data=data.replace(/Defend Droknar's ForgeS*?/gi, '保卫卓克纳熔炉');
      data=data.replace(/Lutgardis ConservatoryS*?/gi, '路嘉蒂斯温室');
      data=data.replace(/Melandru's HopeS*?/gi, '梅兰朵的盼望');
      data=data.replace(/Echovald ForestS*?/gi, '石化森林');
      data=data.replace(/Dredge IncisorS*?/gi, '挖掘者之牙');
      data=data.replace(/DredgeS*?/gi, '挖掘者');
      data=data.replace(/Grendich CourthouseS*?/gi, '葛兰迪法院');
      data=data.replace(/Diessa LowlandsS*?/gi, '底耶沙低地');

      data=data.replace(/Flash GargoyleS*?/gi, '迅速石像鬼');
      data=data.replace(/Shatter GargoyleS*?/gi, '破碎石像鬼');
      data=data.replace(/Resurrect GargoyleS*?/gi, '复活石像鬼');
      data=data.replace(/Singed Gargoyle SkullS*?/gi, '烧焦的石像鬼头颅');
      data=data.replace(/Pogahn PassageS*?/gi, '波甘驿站');
      data=data.replace(/Gandara, the Moon FortressS*?/gi, '弦月要塞 干达拉');
      data=data.replace(/Kournan militaryS*?/gi, '高楠士兵');
      data=data.replace(/Kournan PendantS*?/gi, '高楠垂饰');
	  data=data.replace(/Kourna/gi, '高楠');
      data=data.replace(/Sunjiang DistrictS*?/gi, '孙江行政区');
      data=data.replace(/Shenzun TunnelsS*?/gi, '申赞通道');
      data=data.replace(/Plant FiberS*?/gi, '植物纤维');
      data=data.replace(/Tempered Glass VialS*?/gi, '调合后的玻璃瓶');
      data=data.replace(/Kaineng CenterS*?/gi, '凯宁中心');
      data=data.replace(/Vials*? of InkS*?/gi, '小瓶油水');
      data=data.replace(/Camp RankorS*?/gi, '蓝口营地');
      data=data.replace(/Talus ChuteS*?/gi, '碎石坡道');
      data=data.replace(/Mountain Troll TuskS*?/gi, '山巨魔獠牙');
      data=data.replace(/Mountain TrollS*?/gi, '山巨魔');
      data=data.replace(/Kodonur CrossroadsS*?/gi, '科登诺路口');
      data=data.replace(/Dejarin EstateS*?/gi, '达贾林庄');
      data=data.replace(/Blue Tongue HeketS*?/gi, '蓝舌哈克蛙');
      data=data.replace(/Beast Sworn HeketS*?/gi, '野性哈克蛙');
      data=data.replace(/Blood Cowl HeketS*?/gi, '冷血哈克蛙');
      data=data.replace(/Heket TongueS*?/gi, '哈克蛙舌');
      data=data.replace(/Longeye's LedgeS*?/gi, '长眼岩脉');
      data=data.replace(/Bjora MarchesS*?/gi, '碧拉边境');
      data=data.replace(/Jotun SkullsmasherS*?/gi, '碎骨角顿');
      data=data.replace(/Jotun MindbreakerS*?/gi, '断绪角顿');
      data=data.replace(/Jotun BladeturnerS*?/gi, '转刃角顿');
      data=data.replace(/Jotun PeltS*?/gi, '角顿皮毛');
      data=data.replace(/Harvest TempleS*?/gi, '丰收神殿');
      data=data.replace(/Unwaking WatersS*? \(explorable area\)/gi, '沉睡之水 (探索区)');
  	  data=data.replace(/Unwaking WatersS*?/gi, '沉睡之水');
      data=data.replace(/Saltspray DragonS*?/gi, '盐雾之龙');
      data=data.replace(/Rockhide DragonS*?/gi, '岩皮之龙');
      data=data.replace(/Azure CrestS*?/gi, '湛蓝羽冠');
      data=data.replace(/Yak's BendS*?/gi, '牦牛村');
      data=data.replace(/HydraS*?/gi, '三头龙');
      data=data.replace(/Leathery ClawS*?/gi, '强韧的爪');
      data=data.replace(/Grand Court of SebelkehS*?/gi, '希贝克大宫廷');
      data=data.replace(/The Mirror of LyssS*?/gi, '丽之镜湖');
      data=data.replace(/Roaring Ether HeartS*?/gi, '苍穹咆啸者之心');
      data=data.replace(/Senji's CornerS*?/gi, '山吉之街');
      data=data.replace(/Xaquang SkywayS*?/gi, '夏安便道');
      data=data.replace(/Vermin HideS*?/gi, '寄生虫皮革');
  	  data=data.replace(/VerminS*?/gi, '寄生虫');
      data=data.replace(/Sunspear Great HallS*?/gi, '日戟大会堂');
      data=data.replace(/Plains of JarinS*?/gi, '贾林平原');
      data=data.replace(/Fanged IbogaS*?/gi, '尖牙伊波茄');
      data=data.replace(/Iboga PetalS*?/gi, '伊波茄花瓣');
      data=data.replace(/Champion's DawnS*?/gi, '勇士曙光');
      data=data.replace(/Chef VolonS*?/gi, '大厨 瓦隆');
      data=data.replace(/Pahnai SaladS*?/gi, '伊波茄沙拉');
      data=data.replace(/Seitung HarborS*?/gi, '青函港');
      data=data.replace(/Jaya BluffsS*?/gi, '蛇野断崖');
      data=data.replace(/Mountain YetiS*?/gi, '山雪怪');
      data=data.replace(/Longhair YetiS*?/gi, '长毛雪怪');
      data=data.replace(/Red YetiS*?/gi, '红雪怪');
      data=data.replace(/Stolen SuppliesS*?/gi, '失窃的补给品');
      data=data.replace(/Aurora GladeS*?/gi, '欧若拉林地');
      data=data.replace(/Ettin's BackS*?/gi, '双头怪隐匿地');
      data=data.replace(/Dry TopS*?/gi, '干燥高地');
      data=data.replace(/Nicholas the TravelerS*?/gi, '地图');
      data=data.replace(/Thorn StalkerS*?/gi, '棘刺潜行者');
      data=data.replace(/Tangled SeedS*?/gi, '纠结的种子');
      data=data.replace(/Iron Mines of MoladuneS*?/gi, '莫拉登矿山');
      data=data.replace(/Frozen ForestS*?/gi, '冰冻森林');
      data=data.replace(/PinesoulS*?/gi, '松木怪');
      data=data.replace(/Alpine SeedS*?/gi, '柏木种子');
      data=data.replace(/Gadd's EncampmentS*?/gi, '盖德营区');
      data=data.replace(/Sparkfly SwampS*?/gi, '火星蝇沼泽');
      data=data.replace(/Bogroot GrowthsS*?/gi, '泥塘根源地');
      data=data.replace(/HeketS*?/gi, '青蛙族群');
      data=data.replace(/Rata SumS*?/gi, '洛达顶点');
      data=data.replace(/Magus StonesS*?/gi, '马古斯之石');
      data=data.replace(/Oola's LabS*?/gi, '呜拉实验室');
      data=data.replace(/Hylek AminiS*?/gi, '海格克 阿纳尼');
      data=data.replace(/Hylek NahualliS*?/gi, '海格克 纳猾里');
      data=data.replace(/Hylek TlamatiniS*?/gi, '海格克 拉玛提尼');
      data=data.replace(/Amphibian TongueS*?/gi, '双面人的舌头');
      data=data.replace(/Kodonur CrossroadsS*?/gi, '科登诺路口');
      data=data.replace(/The Floodplain of MahnkelonS*?/gi, '曼克隆泛滥平原');
      data=data.replace(/Embark BeachS*?/gi, '征途海滩');
      data=data.replace(/MerchantS*?/gi, '杂货商人');
      data=data.replace(/Dwarven AleS*?/gi, '矮人啤酒');
      data=data.replace(/Gunnar's HoldS*?/gi, '甘拿的占领地');
      data=data.replace(/Kilroy StonekinS*?/gi, '基罗伊石族');
      data=data.replace(/Fronis Irontoe's LairS*?/gi, '铁趾 佛朗尼的巢穴');
      data=data.replace(/Irontoe'*?s*? ChestS*?/gi, '最终宝箱');
      data=data.replace(/Eredon TerraceS*?/gi, '雷尔登平地');
      data=data.replace(/Maishang HillsS*?/gi, '麦尚山丘');
      data=data.replace(/Pongmei ValleyS*?/gi, '朋美谷');
      data=data.replace(/Islands*? GuardianS*?/gi, '岛屿守护者');
      data=data.replace(/Guardian MossS*?/gi, '守护者苔');
      data=data.replace(/Gate of DesolationS*?/gi, '荒芜之地入口');
      data=data.replace(/Turai's ProcessionS*?/gi, '托雷长廊');
      data=data.replace(/Water Djinn EssenceS*?/gi, '水巨灵精华');
      data=data.replace(/Water DjinnS*?/gi, '水巨灵');
      data=data.replace(/Hulking Stone ElementalS*?/gi, '巨石元素');
      data=data.replace(/Scorched LodestoneS*?/gi, '烧焦的磁石');
      data=data.replace(/Amatz BasinS*?/gi, '亚马兹盆地');
      data=data.replace(/Mourning Veil FallsS*?/gi, '哀伤之幕瀑布');
      data=data.replace(/Rare material traderS*?/gi, '稀有材料商');
      data=data.replace(/Tempered Glass VialS*?/gi, '调和后的玻璃瓶');
      data=data.replace(/Bergen Hot SpringsS*?/gi, '卑尔根温泉');
      data=data.replace(/Nebo TerraceS*?/gi, '尼伯山丘');
      data=data.replace(/Cursed LandsS*?/gi, '诅咒之地');
      data=data.replace(/Skeleton RangerS*?/gi, '骷髅游侠');
      data=data.replace(/Skeleton SorcererS*?/gi, '骷髅巫师');
      data=data.replace(/Grasping GhoulS*?/gi, '贪婪的食尸鬼');
      data=data.replace(/Zombie WarlockS*?/gi, '僵尸法魔');
      data=data.replace(/Skeleton BowmasterS*?/gi, '骷髅弓箭手');
      data=data.replace(/Decayed Orr EmblemS*?/gi, '腐烂的欧尔纹章');
      data=data.replace(/BeetletunS*?/gi, '甲虫镇');
      data=data.replace(/Watchtower CoastS*?/gi, '瞭望塔海岸');
      data=data.replace(/Gates of KrytaS*?/gi, '科瑞塔关所');
      data=data.replace(/Scoundrel's RiseS*?/gi, '恶汉山丘');
      data=data.replace(/Mergoyle WavebreakerS*?/gi, '碎浪石像魔');
      data=data.replace(/Mergoyle SkullS*?/gi, '石像魔头颅');
      data=data.replace(/Nundu BayS*?/gi, '纳度湾');
      data=data.replace(/Marga CoastS*?/gi, '马加海岸');
      data=data.replace(/Yohlon HavenS*?/gi, '犹朗避难所');
      data=data.replace(/Arkjok WardS*?/gi, '阿尔科监禁区');
      data=data.replace(/Bladed Veldt TermiteS*?/gi, '利刃草原蚁');
      data=data.replace(/Veldt Beetle LanceS*?/gi, '草原尖刺甲虫');
      data=data.replace(/Veldt Beetle QueenS*?/gi, '草原甲虫后');
      data=data.replace(/Insect CarapaceS*?/gi, '甲虫壳');
      data=data.replace(/CavalonS*?/gi, '卡瓦隆');
      data=data.replace(/ArchipelagosS*?/gi, '群岛');
      data=data.replace(/Creeping CarpS*?/gi, '爬行鲤鱼');
      data=data.replace(/Scuttle FishS*?/gi, '甲板鱼');
      data=data.replace(/IrukandjiS*?/gi, '毒水母');
      data=data.replace(/Black PearlS*?/gi, '黑珍珠');
      data=data.replace(/Bone PalaceS*?/gi, '白骨宫殿');
      data=data.replace(/Joko's DomainS*?/gi, '杰格领地');
      data=data.replace(/The Shattered RavinesS*?/gi, '碎裂沟谷');
      data=data.replace(/Basalt GrottoS*?/gi, '玄武岩石穴');
      data=data.replace(/Sandstorm CragS*?/gi, '沙风暴．克雷格');
      data=data.replace(/Shambling MesaS*?/gi, '震颤者．梅萨');
      data=data.replace(/Sandblasted LodestoneS*?/gi, '喷沙磁石');
      data=data.replace(/Yak's BendS*?/gi, '牦牛村');
      data=data.replace(/Traveler's ValeS*?/gi, '旅人谷');
      data=data.replace(/Iron Horse MineS*?/gi, '铁马矿山');
      data=data.replace(/Beacon's PerchS*?/gi, '毕肯高地');
      data=data.replace(/Deldrimor BowlS*?/gi, '戴尔迪摩盆地');
      data=data.replace(/Griffon's MouthS*?/gi, '狮鹫兽隘口');
      data=data.replace(/Snow EttinS*?/gi, '冰雪双头巨人');
      data=data.replace(/Icy HumpS*?/gi, '冰雪瘤');
      data=data.replace(/Ran Musu GardensS*?/gi, '岚穆苏花园');
      data=data.replace(/Minister Cho's EstateS*? \(explorable area\)/gi, '周大臣庄园 (探索区)');
    	data=data.replace(/Minister Cho's EstateS*?/gi, '周大臣庄园');
      data=data.replace(/Sickened ServantS*?/gi, '病变的使者');
      data=data.replace(/Sickened PeasantS*?/gi, '病变的书记');
      data=data.replace(/Sickened Guard (warrior)S*?/gi, '病变的警卫');
      data=data.replace(/Forgotten Trinket Boxe*?S*?/gi, '被遗忘的小箱子');
      data=data.replace(/Ventari's RefugeS*?/gi, '凡特里庇护所');
      data=data.replace(/Ettin's BackS*?/gi, '双头怪隐匿第');
      data=data.replace(/Reed BogS*?/gi, '芦苇沼泽地');
      data=data.replace(/The FallsS*?/gi, '陷落区');
      data=data.replace(/Maguuma SpiderS*?/gi, '梅古玛蜘蛛');
      data=data.replace(/Maguuma Spider WebS*?/gi, '梅古玛蜘蛛丝');
      data=data.replace(/Jennur's HordeS*?/gi, '征纳群落');
      data=data.replace(/Vehjin MinesS*?/gi, '威金矿坑');
      data=data.replace(/Cobalt ScabaraS*?/gi, '深蓝斯卡巴拉');
      data=data.replace(/Cobalt MokeleS*?/gi, '深蓝魔克雷');
      data=data.replace(/Cobalt ShriekerS*?/gi, '深蓝尖啸者');
      data=data.replace(/Cobalt TalonS*?/gi, '深蓝之爪');
      data=data.replace(/Sunspear SanctuaryS*?/gi, '日戟避难所');
      data=data.replace(/Command PostS*?/gi, '指挥所');
      data=data.replace(/Jahai BluffsS*?/gi, '夏亥峭壁');
      data=data.replace(/Rare material traderS*?/gi, '稀有材料商人');
      data=data.replace(/Elonian Leathers*(?: SquareS*?)?/gi, '伊洛那皮革');
      data=data.replace(/Deldrimor War CampS*?/gi, '戴尔狄摩兵营');
      data=data.replace(/Grenth's FootprintS*?/gi, '古兰斯的足迹');
      data=data.replace(/Sorrow's FurnaceS*?/gi, '哀伤熔炉');
      data=data.replace(/Priest of SorrowsS*?/gi, '哀伤祭司');
      data=data.replace(/Summit WardenS*?/gi, '石峰看守者');
      data=data.replace(/Summit SurveyorS*?/gi, '石峰测量员');
      data=data.replace(/Summit Dark BinderS*?/gi, '石峰黑暗束缚者');
      data=data.replace(/Summit Deep KnightS*?/gi, '石峰深渊骑士');
      data=data.replace(/Summit TaskmasterS*?/gi, '石峰工头');
      data=data.replace(/Enslavement StoneS*?/gi, '奴隶石');
      data=data.replace(/Ventari's RefugeS*?/gi, '凡特里避难所');
      data=data.replace(/Ettin's BackS*?/gi, '双头怪隐匿地');
      data=data.replace(/The WildsS*?/gi, '荒原');
      data=data.replace(/Moss ScarabS*?/gi, '苔圣甲虫');
      data=data.replace(/Mossy MandibleS*?/gi, '生苔下颚骨');
      data=data.replace(/Ran Musu GardensS*?/gi, '岚穆苏花园');
      data=data.replace(/Kinya ProvinceS*?/gi, '欣弥领地');
      data=data.replace(/Panjiang PeninsulaS*?/gi, '班让半岛');
      data=data.replace(/Crimson SkullS*?/gi, '红颅');
      data=data.replace(/Copper Crimson Skull CoinS*?/gi, '红颅铜币');
      data=data.replace(/Dunes of DespairS*?/gi, '绝望沙丘');
      data=data.replace(/Vulture DriftsS*?/gi, '秃鹰沙丘');
      data=data.replace(/Enchanted HammerS*?/gi, '附魔巨锤兵');
      data=data.replace(/Enchanted SwordS*?/gi, '附魔长剑兵');
      data=data.replace(/Enchanted BowS*?/gi, '附魔弓兵');
      data=data.replace(/Forgotten SealS*?/gi, '遗忘者图章');
      data=data.replace(/Dasha VestibuleS*?/gi, '达沙走廊');
      data=data.replace(/The Hidden City of AhdashimS*?/gi, '隐藏之城 哈达辛');
      data=data.replace(/Key of AhdashimS*?/gi, '哈达辛之钥');
      data=data.replace(/Diamond Djinn EssenceS*?/gi, '钻石巨灵精华');
      data=data.replace(/Diamond DjinnS*?/gi, '钻石巨灵');
      data=data.replace(/Temple of the AgesS*?/gi, '世纪神殿');
      data=data.replace(/Talmark WildernessS*?/gi, '突马克荒地');
      data=data.replace(/Bergen Hot SpringsS*?/gi, '卑尔根温泉');
      data=data.replace(/Cursed LandsS*?/gi, '诅咒之地');
      data=data.replace(/Ancient OakheartS*?/gi, '古老橡树妖');
      data=data.replace(/OakheartS*?/gi, '橡树妖');
      data=data.replace(/Spined AloeS*?/gi, '突刺芦荟');
      data=data.replace(/Reed StalkerS*?/gi, '芦苇潜行者');
      data=data.replace(/Abnormal SeedS*?/gi, '畸形的种子');
      data=data.replace(/The Mouth of TormentS*?/gi, '苦痛之地隘口');
      data=data.replace(/The Ruptured HeartS*?/gi, '破裂之心');
      data=data.replace(/Gate of TormentS*?/gi, '苦痛之门');
      data=data.replace(/Realm of TormentS*?/gi, '苦痛领域');
      data=data.replace(/Nightfallen JahaiS*?/gi, '夜蚀暗殒 夏亥');
      data=data.replace(/Arm of InsanityS*?/gi, '狂乱武装');
      data=data.replace(/Scythe of ChaosS*?/gi, '混沌镰刀');
      data=data.replace(/Blade of CorruptionS*?/gi, '堕落之刃');
      data=data.replace(/Shadow of FearS*?/gi, '恐惧暗影');
      data=data.replace(/Rain of TerrorS*?/gi, '惊骇之雨');
      data=data.replace(/Herald of NightmaresS*?/gi, '梦靥使者');
      data=data.replace(/Spear of TormentS*?/gi, '苦痛之矛');
      data=data.replace(/Word of MadnessS*?/gi, '疯狂话语');
      data=data.replace(/Demonic RelicS*?/gi, '恶魔残片');
      data=data.replace(/Ice Tooth CaveS*?/gi, '冰牙洞穴');
      data=data.replace(/Anvil RockS*?/gi, '铁砧石');
      data=data.replace(/Frostfire DryderS*?/gi, '霜火蛛化精灵');
      data=data.replace(/Frostfire FangS*?/gi, '霜火尖牙');
      ...
      data=data.replace(/Rot WallowS*?/gi, '腐败兽');
      data=data.replace(/Rot Wallow TuskS*?/gi, '腐败兽獠牙');
      data=data.replace(/Elona ReachS*?/gi, '伊洛那流域');
      data=data.replace(/Diviner's AscentS*?/gi, '预言者之坡');
      data=data.replace(/Sand DrakeS*?/gi, '沙龙兽');
      data=data.replace(/Topaz CrestS*?/gi, '黄宝石颈脊');
      data=data.replace(/Rata SumS*?/gi, '洛达顶点');
      data=data.replace(/Magus StonesS*?/gi, '玛古斯之石');
      data=data.replace(/LifeweaverS*?/gi, '织命者');
      data=data.replace(/BloodweaverS*?/gi, '织血者');
      data=data.replace(/VenomweaverS*?/gi, '织恨者');
      data=data.replace(/Weaver LegS*?/gi, '编织者的腿');
      data=data.replace(/Yahnur MarketS*?/gi, '雅诺尔市集');
      data=data.replace(/Vehtendi ValleyS*?/gi, '巍天帝峡谷');
      data=data.replace(/Storm JacarandaS*?/gi, '暴风荆棘');
      data=data.replace(/Mirage IbogaS*?/gi, '幻象伊波枷');
      data=data.replace(/Enchanted BramblesS*?/gi, '魔法树根');
      data=data.replace(/Whistling ThornbrushS*?/gi, '荆棘之藤');
      data=data.replace(/Sentient SporeS*?/gi, '知觉孢子');
      data=data.replace(/AhojS*?/gi, '亚禾');
      data=data.replace(/Bottle of Vabbian WineS*?/gi, '瓦贝红酒');
      data=data.replace(/Jarimiya the UnmercifulS*?/gi, '残酷 贾米里');
      data=data.replace(/Blacktide DenS*?/gi, '黑潮之穴');
      data=data.replace(/Lahtenda BogS*?/gi, '洛天帝沼泽');
      data=data.replace(/Mandragor ImpS*?/gi, '曼陀罗恶魔');
      data=data.replace(/Mandragor SlitherS*?/gi, '曼陀罗之藤');
      data=data.replace(/Stoneflesh MandragorS*?/gi, '曼陀罗石根');
      data=data.replace(/Mandragor SwamprootS*?/gi, '曼陀罗根');
      ...
      data=data.replace(/Skill Hungry GakiS*?/gi, '灵巧的饿鬼');
      data=data.replace(/Pain Hungry GakiS*?/gi, '痛苦的饿鬼');
      data=data.replace(/Quarrel FallsS*?/gi, '怨言瀑布');
      data=data.replace(/SilverwoodS*?/gi, '银树');
      data=data.replace(/Maguuma WarriorS*?/gi, '梅古玛战士');
      data=data.replace(/Maguuma HunterS*?/gi, '梅古玛猎人');
      data=data.replace(/Maguuma ProtectorS*?/gi, '梅古玛守护者');
      ...
      data=data.replace(/Seeker's PassageS*?/gi, '探索者通道');
      data=data.replace(/Salt FlatsS*?/gi, '盐滩');
      data=data.replace(/The Amnoon OasisS*?/gi, '安努绿洲');
      data=data.replace(/Prophet's PathS*?/gi, '先知之路');
      data=data.replace(/Jade ScarabS*?/gi, '翡翠圣甲虫');
      data=data.replace(/Jade MandibleS*?/gi, '翡翠下颚骨');
      data=data.replace(/Temple of the AgesS*?/gi, '辛库走廊');
      data=data.replace(/Sunjiang DistrictS*?/gi, '孙江行政区');
      data=data.replace(/Sunjiang DistrictS*?/gi, '孙江行政区');
      data=data.replace(/Shenzun TunnelsS*?/gi, '申赞通道');
      data=data.replace(/AfflictedS*?/gi, '被感染的');
      data=data.replace(/Putrid CystS*?/gi, '腐败胞囊');
      data=data.replace(/The AstralariumS*?/gi, '世纪神殿');
      data=data.replace(/Zehlon ReachS*?/gi, '黑色帷幕');
      data=data.replace(/Beknur HarborS*?/gi, '突马克荒地');
      data=data.replace(/Skale BlighterS*?/gi, '森林牛头怪');
      //data=data.replace(/Skale FinS*?/gi, '森林牛头怪的角');
      data=data.replace(/The AstralariumS*?/gi, '亚斯特拉利姆');
      data=data.replace(/Zehlon ReachS*?/gi, '列隆流域');
      data=data.replace(/Beknur HarborS*?/gi, '别克诺港');
      data=data.replace(/Issnur IslesS*?/gi, '伊斯诺岛');
      ...
      data=data.replace(/Behemoth JawS*?/gi, '巨兽颚');
      data=data.replace(/Root BehemothS*?/gi, '根巨兽');
      data=data.replace(/Brauer AcademyS*?/gi, '袭哈拉');
      data=data.replace(/Jaga MoraineS*?/gi, '亚加摩瑞恩');
      data=data.replace(/UndergrowthS*?/gi, '海冶克狂战士');
      data=data.replace(/Dragon MossS*?/gi, '牛头怪狂战士');
      data=data.replace(/Dragon MossS*?/gi, '狂战士 纹帝哥');
      data=data.replace(/Dragon MossS*?/gi, '棘狼狂战士');
      data=data.replace(/Berserker HornS*?/gi, '狂战士的角');
      data=data.replace(/Brauer AcademyS*?/gi, '巴尔学院');
      data=data.replace(/Drazach ThicketS*?/gi, '德瑞扎灌木林');
      data=data.replace(/Tanglewood CopseS*?/gi, '谭格坞树林');
      ...
      data=data.replace(/Drakkar LakeS*?/gi, '卓卡湖');
      data=data.replace(/Frozen ElementalS*?/gi, '冰元素');
      data=data.replace(/Pile of Elemental DustS*?/gi, '元素之土');
      data=data.replace(/Bergen Hot SpringsS*?/gi, '卑尔根温泉');
      data=data.replace(/Nebo TerraceS*?/gi, '尼伯山丘');
      data=data.replace(/North Kryta ProvinceS*?/gi, '科瑞塔北部');
      data=data.replace(/Gypsie EttinS*?/gi, '流浪双头巨人');
      data=data.replace(/Hardened HumpS*?/gi, '硬瘤');
      data=data.replace(/Leviathan PitsS*?/gi, '利拜亚森矿场');
      data=data.replace(/Silent SurfS*?/gi, '寂静之浪');
      data=data.replace(/Seafarer's RestS*?/gi, '航海者休憩处');
      data=data.replace(/OniS*?/gi, '鬼');
      data=data.replace(/Keen Oni TalonS*?/gi, '锐利细鬼爪');
      data=data.replace(/Ice Caves of SorrowS*?/gi, '悲伤冰谷');
      data=data.replace(/IcedomeS*?/gi, '冰点');
      data=data.replace(/Ice ElementalS*?/gi, '冰元素');
      data=data.replace(/Ice GolemS*?/gi, '冰高仑');
      data=data.replace(/Icy LodestoneS*?/gi, '冰磁石');
      data=data.replace(/Augury RockS*?/gi, '占卜之石');
      data=data.replace(/Skyward ReachS*?/gi, '天际流域');
      data=data.replace(/Destiny's GorgeS*?/gi, '命运峡谷');
      data=data.replace(/Prophet's PathS*?/gi, '探索者通道');
      data=data.replace(/Salt FlatsS*?/gi, '盐滩');
      data=data.replace(/Storm KinS*?/gi, '风暴魔');
      data=data.replace(/Shriveled EyeS*?/gi, '干枯的眼睛');
      data=data.replace(/Skull JujuS*?/gi, '头骨土符');
      data=data.replace(/Skull JujuS*?/gi, '颅骨土符');
      data=data.replace(/Bone CharmS*? \(trophy\)/gi, '骨制护符');
  	  data=data.replace(/Bone CharmS*?/gi, '骨制护符');

  	//data=data.replace(/Note:/gi, '注:');
  	//data=data.replace(/Nicholas' location and requested item changes weekly on Monday at 15:00 UTC./gi, '每周循环 于以下各晚11点起 生效');
  	//data=data.replace(/If your browser does not update the item and location, you can <a rel="nofollow" class="external text" http:\/\/wiki.guildwars.com\/index.php\?title=Nicholas_the_Traveler\/Cycle&amp;action=purge">refresh the list<\/a>\./gi, '');


  	data=data.replace(/January/gi, '一月');
  	data=data.replace(/February/gi, '二月');
  	data=data.replace(/March/gi, '三月');
  	data=data.replace(/April/gi, '四月');
  	data=data.replace(/May/gi, '五月');
  	data=data.replace(/June/gi, '六月');
  	data=data.replace(/July/gi, '七月');
  	data=data.replace(/August/gi, '八月');
  	data=data.replace(/September/gi, '九月');
  	data=data.replace(/October/gi, '十月');
  	data=data.replace(/November/gi, '十一月');
  	data=data.replace(/December/gi, '十二月');

    data=data.replace(/Prophecies/gi, '一章');
  	data=data.replace(/Factions/gi, '二章');
  	data=data.replace(/Nightfall/gi, '三章');
  	data=data.replace(/Eye of the North/gi, '四章');
    data=data.replace(/Kryta/gi, '科瑞塔');

    data=data.replace(/SpiderS*?/gi, '蜘蛛');
    data=data.replace(/CharrS*?/gi, '夏尔');
    data=data.replace(/Map/gi, '路线');
    if (kAd){
      data=data.replace(/[sS]/gi,'');
      data=data.replace(/\s*/gi,'');
    }
    return data;
  }

  var monthConversion1 = function(data){
  ...
  var parseTranslate = function(data){
      ...
      //data=data.replace(/(^|[^A-Za-z])(FAITH IS MY SHIELD)(?=[^A-Za-z]|$)/gi, '$1"___"');
      //data=data.replace(/(^|[^A-Za-z])(MIGHT MAKES RIGHT)(?=[^A-Za-z]|$)/gi, '$1"___"');
      ...
      //data=data.replace(/(^|[^A-Za-z])(DOWN BUT NOT OUT)(?=[^A-Za-z]|$)/gi, '$1"___"');
    ...
    data=data.replace(/Gargoyle SkullS*?/gi, '石像鬼头颅');
      ...
      data=data.replace(/(^|[^A-Za-z])(Echovald ForestS*?)(?=[^A-Za-z]|$)/gi, '$1石化森林');
      ...
      data=data.replace(/GargoyleS*?/gi, '石像鬼');
      data=data.replace(/(^|[^A-Za-z])(Pogahn PassageS*?)(?=[^A-Za-z]|$)/gi, '$1波甘驿站');
      ...
      data=data.replace(/Roaring EtherS*?/gi, '苍穹咆啸者');
      data=data.replace(/Roaring EtherS*?/gi, '苍穹咆哮者');
  	...
  	data=data.replace(/(^|[^A-Za-z])(永久 CHAMPI*?O*?N*?S*?(?: OF )?(BAL*?THA*?Z*?R*?A*?R*?D*?'?s*?Z*?)?)(?=[^A-Za-z]|$)/gi, '$1永久变身 巴萨泽拥护者 [http://wiki.guildwars.com/wiki/Everlasting_Balthazar%27s_Champion_Tonic] [http://wiki.guildwars.com/wiki/Champion_of_Balthazar]');

  	data=data.replace(/(^|[^A-Za-z])(CHAMPI*?O*?N*?S*? (?:OF )?BAL*?THA*?Z*?R*?A*?R*?D*?'?s*?Z*?|BAL*?THA*?Z*?R*?A*?R*?D*?'?s*?Z*? CHAMPIONS*?)(?=[^A-Za-z]|$)/gi, '$1巴萨泽拥护者 [http://wiki.guildwars.com/wiki/Everlasting_Balthazar%27s_Champion_Tonic] [http://wiki.guildwars.com/wiki/Champion_of_Balthazar] ');
  	data=data.replace(/(^|[^A-Za-z])(PRIESTS*? (?:OF )?BAL*?THA*?Z*?R*?A*?R*?D*?'?s*?Z*?)(?=[^A-Za-z]|$)/gi, '$1巴萨泽的祭司 [http://wiki.guildwars.com/wiki/Everlasting_Priest_of_Balthazar_Tonic] [http://wiki.guildwars.com/wiki/Priest_of_Balthazar] ');
  	data=data.replace(/(^|[^A-Za-z])(AVATARS*? (?:OF )?BAL*?THA*?Z*?R*?A*?R*?D*?'?s*?Z*?)(?=[^A-Za-z]|$)/gi, '$1巴萨泽化身 [http://wiki.guildwars.com/wiki/Everlasting_Avatar_of_Balthazar_Tonic] [http://wiki.guildwars.com/wiki/Avatar_of_Balthazar_Form] ');
  	data=data.replace(/(^|[^A-Za-z])(GH*?OSTLY HEROE*?S*?)(?=[^A-Za-z]|$)/gi, '$1英雄之魂 [http://wiki.guildwars.com/wiki/Everlasting_Ghostly_Hero_Tonic] [http://wiki.guildwars.com/wiki/Ghostly_Hero_Form] ');
  	data=data.replace(/(^|[^A-Za-z])(GH*?OSTLY PRIESTS*?)(?=[^A-Za-z]|$)/gi, '$1游魂祭司 [http://wiki.guildwars.com/wiki/Everlasting_Ghostly_Priest_Tonic] [http://wiki.guildwars.com/wiki/Ghostly_Priest] ');
  	data=data.replace(/(^|[^A-Za-z])(GUILD LORDS*?)(?=[^A-Za-z]|$)/gi, '$1公会领主 [http://wiki.guildwars.com/wiki/Everlasting_Guild_Lord_Tonic] [http://wiki.guildwars.com/wiki/Guild_Lord_Form] ');
  	data=data.replace(/(^|[^A-Za-z])(SINISTER AUTOMATONICS*?)(?=[^A-Za-z]|$)/gi, '$1黑高仑 [http://wiki.guildwars.com/wiki/Everlasting_Sinister_Automatonic] [http://wiki.guildwars.com/wiki/Sinister_Golem_Form] ');
  	data=data.replace(/(^|[^A-Za-z\_])(AUTOMATONICS*?)(?=[^A-Za-z]|$)/gi, '$1高仑 [http://wiki.guildwars.com/wiki/Everlasting_Automatonic] [http://wiki.guildwars.com/wiki/Golem_Form] ');
  ...
  var gCnEn = function(data){

	data=data.replace(/士气提升/gi, "Morale Boost");
	data=data.replace(/士气/gi, "Morale");
	data=data.replace(/核/gi, "Core");
	data=data.replace(/一章/gi, "Prophecies");
	data=data.replace(/二章/gi, "Factions");
	data=data.replace(/三章/gi, "Nightfall");
	data=data.replace(/四章/gi, "Eye of the North");
	data=data.replace(/光明使者/gi, "Lightbringer");
	data=data.replace(/日戟/gi, "Sunspear");
	data=data.replace(/黑檀先锋队/gi, "Ebon Vanguard");
	data=data.replace(/诺恩/gi, "Norn");
	data=data.replace(/阿苏拉/gi, "Asura");
	data=data.replace(/戴尔狄摩/gi, "Deldrimor");

	data=data.replace(/快速施法/gi, "Fast Casting");
	data=data.replace(/幻术魔法/gi, "Illusion Magic");
	data=data.replace(/支配魔法/gi, "Domination Magic");
	data=data.replace(/灵感魔法/gi, "Inspiration Magic");
	data=data.replace(/血魔法/gi, "Blood Magic");
	data=data.replace(/死亡魔法/gi, "Death Magic");
	data=data.replace(/灵魂吸取/gi, "Soul Reaping");
	data=data.replace(/诅咒/gi, "Curses");
	data=data.replace(/风系魔法/gi, "Air Magic");
	data=data.replace(/地系魔法/gi, "Earth Magic");
	data=data.replace(/火系魔法/gi, "Fire Magic");
	data=data.replace(/水系魔法/gi, "Water Magic");
	data=data.replace(/能量储存/gi, "Energy Storage");
	data=data.replace(/治疗/gi, "Healing Prayers");
	data=data.replace(/惩戒/gi, "Smiting Prayers");
	data=data.replace(/防护/gi, "Protection Prayers");
	data=data.replace(/神恩/gi, "Divine Favor");
	data=data.replace(/力量/gi, "Strength");
	data=data.replace(/斧术/gi, "Axe Mastery");
	data=data.replace(/锤术/gi, "Hammer Mastery");
	data=data.replace(/剑术/gi, "Swordsmanship");
	data=data.replace(/战术/gi, "Tactics");
	data=data.replace(/野兽术/gi, "Beast Mastery");
	data=data.replace(/专精/gi, "Expertise");
	data=data.replace(/求生/gi, "Wilderness Survival");
	data=data.replace(/弓术/gi, "Marksmanship");
	data=data.replace(/匕首术/gi, "Dagger Mastery");
	data=data.replace(/暗杀技巧/gi, "Deadly Arts");
	data=data.replace(/暗影技巧/gi, "Shadow Arts");
	data=data.replace(/神谕/gi, "Communing");
	data=data.replace(/复原魔法/gi, "Restoration Magic");
	data=data.replace(/导引魔法/gi, "Channeling Magic");
	data=data.replace(/致命攻击/gi, "Critical Strikes");
	data=data.replace(/召唤/gi, "Spawning Power");
	data=data.replace(/矛术/gi, "Spear Mastery");
	data=data.replace(/命令/gi, "Command");
	data=data.replace(/激励/gi, "Motivation");
	data=data.replace(/领导/gi, "Leadership");
	data=data.replace(/镰刀术/gi, "Scythe Mastery");
	data=data.replace(/风系祷告/gi, "Wind Prayer");
	data=data.replace(/地系祷告/gi, "Earth Prayers");
	data=data.replace(/秘法/gi, "Mysticism");
	data=data.replace(/降咒魔法/gi, "Hex Spell");
	data=data.replace(/碰触魔法/gi, "Touch Spell");
	data=data.replace(/碰触技能/gi, "Touch Skill");
	data=data.replace(/咒文魔法/gi, "Spell");
	data=data.replace(/加持魔法/gi, "Enchantment Spell");
	data=data.replace(/即刻加持魔法/gi, "Flash Enchantment Spell");
	data=data.replace(/近身攻击技能/gi, "Melee Attack");
	data=data.replace(/缚灵仪式/gi, "Binding Ritual");
	data=data.replace(/自然仪式/gi, "Nature Ritual");
	data=data.replace(/战嚎/gi, "Shout");
	data=data.replace(/技能/gi, "Skill");
	data=data.replace(/态势/gi, "Stance");
	data=data.replace(/纹章/gi, "Signet");
	data=data.replace(/符号/gi, "Glyph");
	data=data.replace(/抱持魔法/gi, "Item Spell");
	data=data.replace(/结界魔法/gi, "Ward Spell");
	data=data.replace(/武装魔法/gi, "Weapon Spell");
	data=data.replace(/回响/gi, "Echo");
	data=data.replace(/矛术进攻技能/gi, "Spear Attack");
	data=data.replace(/镰刀进攻技能/gi, "Scythe Attack");

	data=data.replace(/再生秘法/gi, "Mystic Regeneration");
	data=data.replace(/魔幻掩蔽/gi, "Mirage Cloak");
	data=data.replace(/虚晃打击/gi, "Distracting Blow");
	data=data.replace(/惊天神力/gi, "Staggering Force");
	data=data.replace(/力量誓约/gi, "Vow of Strength");
	data=data.replace(/沙尘裂片/gi, "Sand Shards");
	data=data.replace(/回防/gi, "Fall Back");
	data=data.replace(/治疗降咒/gi, "Cure Hex");
	data=data.replace(/勤奋精神/gi, "Patient Spirit");
	data=data.replace(/身体牺牲/gi, "Flesh of My Flesh");
	data=data.replace(/嘈杂/gi, "Technobabble");
	data=data.replace(/爆刺武装/gi, "Splinter Weapon");
	data=data.replace(/灵力召唤/gi, "Summon Spirits");
	data=data.replace(/精神隔绝/gi, "Mental Block");
	data=data.replace(/地之护甲/gi, "Armor of Earth");
	data=data.replace(/黑曜石肉体/gi, "Obsidian Flesh");
	data=data.replace(/裂片护甲/gi, "Sliver Armor");
	data=data.replace(/石之打击/gi, "Stone Striker");
	data=data.replace(/石体灵气/gi, "Stoneflesh Aura");
	data=data.replace(/专注符号/gi, "Glyph of Concentration");
	data=data.replace(/大地颂歌/gi, "Mantra of Earth");
	data=data.replace(/意志颂歌/gi, "Mantra of Resolve");
	data=data.replace(/苦痛幻觉/gi, "Illusion of Pain");
	data=data.replace(/缚灵纹章/gi, "Signet of Spirits");
	data=data.replace(/活力微风/gi, "Quickening Zephyr");
	data=data.replace(/血歌/gi, "Bloodsong");
	data=data.replace(/感同身受/gi, "Sympathetic Visage");
	data=data.replace(/回复灵气/gi, "Aura of Restoration");
	data=data.replace(/吹拂之风/gi, "Winnowing");
	data=data.replace(/黑檀先锋队暗杀者支援/gi, "Ebon Vanguard Assassin Support");
	data=data.replace(/暗影慰藉/gi, "Shadow Sanctuary");
	data=data.replace(/能力调和/gi, "Elemental Lord");
	data=data.replace(/丽莎灵气/gi, "Lyssa's Aura");
	data=data.replace(/先人的盛怒/gi, "Ancestors' Rage");
	data=data.replace(/生灵裂缝/gi, "Spirit Rift");
	data=data.replace(/隐士攻击/gi, "Eremite's Attack");
	data=data.replace(/别碰我/gi, "Can't Touch This");
	data=data.replace(/薇娜之吻/gi, "Dwayna's Kiss");
	data=data.replace(/超载/gi, "Overload");
	data=data.replace(/回音/gi, "Echo");
	data=data.replace(/恶灵附身/gi, "Spiteful Spirit");
	data=data.replace(/生灵苦痛/gi, "Spiritual Pain");
	data=data.replace(/病症扩散/gi, "Epidemic");
	data=data.replace(/亵渎加持/gi, "Desecrate Enchantments");
	data=data.replace(/瓦思崔的忧虑/gi, "Wastrel's Worry");
	data=data.replace(/蛇之敏捷/gi, "Serpent's Quickness");
	data=data.replace(/辐射场/gi, "Radiation Field");
	data=data.replace(/巴萨则的精神/gi, "Balthazar's Spirit");
	data=data.replace(/瓦斯崔之死/gi, "Wastrel's Demise");
	data=data.replace(/自虐/gi, "Empathy");
	data=data.replace(/恍惚之眼/gi, "Wandering Eye");
	data=data.replace(/索命/gi, "Death's Charge");
	data=data.replace(/危难遮蔽/gi, "Shroud of Distress");
	data=data.replace(/暗影形态/gi, "Shadow Form");
	data=data.replace(/暗影迅速/gi, "Shadow of Haste");
	data=data.replace(/黑暗脱逃/gi, "Dark Escape");
	data=data.replace(/无人能抵/gi, "I Am Unstoppable");
	data=data.replace(/回归/gi, "Recall");
	data=data.replace(/暗影之心/gi, "Heart of Shadow");
	data=data.replace(/优越之息/gi, "Air of Superiority");
	data=data.replace(/懊悔幻想/gi, "Visions of Regret");
	data=data.replace(/痛苦哭喊/gi, "Cry of Pain");
	data=data.replace(/反常纹章/gi, "Unnatural Signet");
	data=data.replace(/你的动作像个矮人/gi, "You Move Like a Dwarf");
	data=data.replace(/幻觉粉碎/gi, "Shatter Delusions");
	data=data.replace(/疑惑/gi, "Mistrust");
	data=data.replace(/灭绝之地/gi, "Edge of Extinction");
	data=data.replace(/魔法回音/gi, "Arcane Echo");
	data=data.replace(/能量震荡/gi, "Energy Surge");
	data=data.replace(/痛苦倒转/gi, "Pain Inverter");
	data=data.replace(/结束他/gi, "Finish Him");
	data=data.replace(/冲突/gi, "Necrosis");
	data=data.replace(/魔力反噬/gi, "Backfire");
	data=data.replace(/生命种子/gi, "Seed of Life");
	data=data.replace(/吸收盾/gi, "Shield of Absorption");
	data=data.replace(/祝福灵气/gi, "Blessed Aura");
	data=data.replace(/生命连结/gi, "Life Bond");
	data=data.replace(/坚毅灵气/gi, "Unyielding Aura");
	data=data.replace(/黑檀脱逃/gi, "Ebon Escape");
	data=data.replace(/速度燃烧/gi, "Burning Speed");
	data=data.replace(/精神连结/gi, "Spirit Bond");
	data=data.replace(/魔力复原/gi, "Ether Renewal");
	data=data.replace(/生之礼赞/gi, "Infuse Health");
	data=data.replace(/保护连结/gi, "Protective Bond");
	data=data.replace(/黑檀战斗荣誉规范/gi, "Ebon Battle Standard of Honor");
	data=data.replace(/旋风防御/gi, "Whirling Defense");
	data=data.replace(/矮人坚定/gi, "Dwarven Stability");

	return data;
  }

  var gEnCn = function(data){
    data=data.replace(/Morale Boost/gi, '士气提升');
	data=data.replace(/Morale/gi, '士气');
	data=data.replace(/Core/gi, '核');
	data=data.replace(/Prophecies/gi, '一章');
	data=data.replace(/Factions/gi, '二章');
	data=data.replace(/Nightfall/gi, '三章');
	data=data.replace(/Eye of the North/gi, '四章');
	data=data.replace(/Eotn/gi, '四章');
	data=data.replace(/Lightbringer/gi, '光明使者');
	data=data.replace(/Sunspear/gi, '日戟');
	data=data.replace(/Ebon Vanguard/gi, '黑檀先锋队');
	data=data.replace(/Norn/gi, '诺恩');
	data=data.replace(/Asura/gi, '阿苏拉');
	data=data.replace(/Deldrimor/gi, '戴尔狄摩');

	data=data.replace(/Fast Casting/gi, '快速施法');
	data=data.replace(/Illusion Magic/gi, '幻术魔法');
	data=data.replace(/Domination Magic/gi, '支配魔法');
	data=data.replace(/Inspiration Magic/gi, '灵感魔法');
	data=data.replace(/Blood Magic/gi, '血魔法');
	data=data.replace(/Death Magic/gi, '死亡魔法');
	data=data.replace(/Soul Reaping/gi, '灵魂吸取');
	data=data.replace(/Curses/gi, '诅咒');
	data=data.replace(/Air Magic/gi, '风系魔法');
	data=data.replace(/Earth Magic/gi, '地系魔法');
	data=data.replace(/Fire Magic/gi, '火系魔法');
	data=data.replace(/Water Magic/gi, '水系魔法');
	data=data.replace(/Energy Storage/gi, '能量储存');
	data=data.replace(/Healing Prayers/gi, '治疗');
	data=data.replace(/Smiting Prayers/gi, '惩戒');
	data=data.replace(/Protection Prayers/gi, '防护');
	data=data.replace(/Divine Favor/gi, '神恩');
	data=data.replace(/Strength/gi, '力量');
	data=data.replace(/Axe Mastery/gi, '斧术');
	data=data.replace(/Hammer Mastery/gi, '锤术');
	data=data.replace(/Swordsmanship/gi, '剑术');
	data=data.replace(/Tactics/gi, '战术');
	data=data.replace(/Beast Mastery/gi, '野兽术');
	data=data.replace(/Expertise/gi, '专精');
	data=data.replace(/Wilderness Survival/gi, '求生');
	data=data.replace(/Marksmanship/gi, '弓术');
	data=data.replace(/Dagger Mastery/gi, '匕首术');
	data=data.replace(/Deadly Arts/gi, '暗杀技巧');
	data=data.replace(/Shadow Arts/gi, '暗影技巧');
	data=data.replace(/Communing/gi, '神谕');
	data=data.replace(/Restoration Magic/gi, '复原魔法');
	data=data.replace(/Channeling Magic/gi, '导引魔法');
	data=data.replace(/Critical Strikes/gi, '致命攻击');
	data=data.replace(/Spawning Power/gi, '召唤');
	data=data.replace(/Spear Mastery/gi, '矛术');
	data=data.replace(/Command/gi, '命令');
	data=data.replace(/Motivation/gi, '激励');
	data=data.replace(/Leadership/gi, '领导');
	data=data.replace(/Scythe Mastery/gi, '镰刀术');
	data=data.replace(/Wind Prayer/gi, '风系祷告');
	data=data.replace(/Earth Prayers/gi, '地系祷告');
	data=data.replace(/Mysticism/gi, '秘法');
	data=data.replace(/Hex Spell/gi, '降咒魔法');
	data=data.replace(/Touch Spell/gi, '碰触魔法');
	data=data.replace(/Touch Skill/gi, '碰触技能');
	data=data.replace(/Spell/gi, '咒文魔法');
	data=data.replace(/Enchantment Spell/gi, '加持魔法');
	data=data.replace(/Flash Enchantment Spell/gi, '即刻加持魔法');
	data=data.replace(/Melee Attack/gi, '近身攻击技能');
	data=data.replace(/Binding Ritual/gi, '缚灵仪式');
	data=data.replace(/Nature Ritual/gi, '自然仪式');
	data=data.replace(/Shout/gi, '战嚎');
	data=data.replace(/Skill/gi, '技能');
	data=data.replace(/Stance/gi, '态势');
	data=data.replace(/Signet/gi, '纹章');
	data=data.replace(/Glyph/gi, '符号');
	data=data.replace(/Item Spell/gi, '抱持魔法');
	data=data.replace(/Ward Spell/gi, '结界魔法');
	data=data.replace(/Weapon Spell/gi, '武装魔法');
	data=data.replace(/Echo/gi, '回响');
	data=data.replace(/Spear Attack/gi, '矛术进攻技能');
	data=data.replace(/Scythe Attack/gi, '镰刀进攻技能');

	data=data.replace(/Mystic Regeneration/gi, '再生秘法');
	data=data.replace(/Mirage Cloak/gi, '魔幻掩蔽');
	data=data.replace(/Distracting Blow/gi, '虚晃打击');
	data=data.replace(/Staggering Force/gi, '惊天神力');
	data=data.replace(/Vow of Strength/gi, '力量誓约');
	data=data.replace(/Sand Shards/gi, '沙尘裂片');
	data=data.replace(/Fall Back/gi, '回防');
	data=data.replace(/Cure Hex/gi, '治疗降咒');
	data=data.replace(/Patient Spirit/gi, '勤奋精神');
	data=data.replace(/Flesh of My Flesh/gi, '身体牺牲');
	data=data.replace(/Technobabble/gi, '嘈杂');
	data=data.replace(/Splinter Weapon/gi, '爆刺武装');
	data=data.replace(/Summon Spirits/gi, '灵力召唤');
	data=data.replace(/Mental Block/gi, '精神隔绝');
	data=data.replace(/Armor of Earth/gi, '地之护甲');
	data=data.replace(/Obsidian Flesh/gi, '黑曜石肉体');
	data=data.replace(/Sliver Armor/gi, '裂片护甲');
	data=data.replace(/Stone Striker/gi, '石之打击');
	data=data.replace(/Stoneflesh Aura/gi, '石体灵气');
	data=data.replace(/Glyph of Concentration/gi, '专注符号');
	data=data.replace(/Mantra of Earth/gi, '大地颂歌');
	data=data.replace(/Mantra of Resolve/gi, '意志颂歌');
	data=data.replace(/Illusion of Pain/gi, '苦痛幻觉');
	data=data.replace(/Signet of Spirits/gi, '缚灵纹章');
	data=data.replace(/Quickening Zephyr/gi, '活力微风');
	data=data.replace(/Bloodsong/gi, '血歌');
	data=data.replace(/Sympathetic Visage/gi, '感同身受');
	data=data.replace(/Aura of Restoration/gi, '回复灵气');
	data=data.replace(/Winnowing/gi, '吹拂之风');
	data=data.replace(/Ebon Vanguard Assassin Support/gi, '黑檀先锋队暗杀者支援');
	data=data.replace(/Shadow Sanctuary/gi, '暗影慰藉');
	data=data.replace(/Elemental Lord/gi, '能力调和');
	data=data.replace(/Lyssa's Aura/gi, '丽莎灵气');
	data=data.replace(/Ancestors' Rage/gi, '先人的盛怒');
	data=data.replace(/Spirit Rift/gi, '生灵裂缝');
	data=data.replace(/Eremite's Attack/gi, '隐士攻击');
	data=data.replace(/Can't Touch This/gi, '别碰我');
	data=data.replace(/Dwayna's Kiss/gi, '薇娜之吻');
	data=data.replace(/Overload/gi, '超载');
	data=data.replace(/Echo/gi, '回音');
	data=data.replace(/Spiteful Spirit/gi, '恶灵附身');
	data=data.replace(/Spiritual Pain/gi, '生灵苦痛');
	data=data.replace(/Epidemic/gi, '病症扩散');
	data=data.replace(/Desecrate Enchantments/gi, '亵渎加持');
	data=data.replace(/Wastrel's Worry/gi, '瓦思崔的忧虑');
	data=data.replace(/Serpent's Quickness/gi, '蛇之敏捷');
	data=data.replace(/Radiation Field/gi, '辐射场');
	data=data.replace(/Balthazar's Spirit/gi, '巴萨则的精神');
	data=data.replace(/Wastrel's Demise/gi, '瓦斯崔之死');
	data=data.replace(/Empathy/gi, '自虐');
	data=data.replace(/Wandering Eye/gi, '恍惚之眼');
	data=data.replace(/Death's Charge/gi, '索命');
	data=data.replace(/Shroud of Distress/gi, '危难遮蔽');
	data=data.replace(/Shadow Form/gi, '暗影形态');
	data=data.replace(/Shadow of Haste/gi, '暗影迅速');
	data=data.replace(/Dark Escape/gi, '黑暗脱逃');
	data=data.replace(/I Am Unstoppable/gi, '无人能抵');
	data=data.replace(/Recall/gi, '回归');
	data=data.replace(/Heart of Shadow/gi, '暗影之心');
	data=data.replace(/Air of Superiority/gi, '优越之息');
	data=data.replace(/Visions of Regret/gi, '懊悔幻想');
	data=data.replace(/Cry of Pain/gi, '痛苦哭喊');
	data=data.replace(/Unnatural Signet/gi, '反常纹章');
	data=data.replace(/You Move Like a Dwarf/gi, '你的动作像个矮人');
	data=data.replace(/Shatter Delusions/gi, '幻觉粉碎');
	data=data.replace(/Mistrust/gi, '疑惑');
	data=data.replace(/Edge of Extinction/gi, '灭绝之地');
	data=data.replace(/Arcane Echo/gi, '魔法回音');
	data=data.replace(/Energy Surge/gi, '能量震荡');
	data=data.replace(/Pain Inverter/gi, '痛苦倒转');
	data=data.replace(/Finish Him/gi, '结束他');
	data=data.replace(/Necrosis/gi, '冲突');
	data=data.replace(/Backfire/gi, '魔力反噬');
	data=data.replace(/Seed of Life/gi, '生命种子');
	data=data.replace(/Shield of Absorption/gi, '吸收盾');
	data=data.replace(/Blessed Aura/gi, '祝福灵气');
	data=data.replace(/Life Bond/gi, '生命连结');
	data=data.replace(/Unyielding Aura/gi, '坚毅灵气');
	data=data.replace(/Ebon Escape/gi, '黑檀脱逃');
	data=data.replace(/Burning Speed/gi, '速度燃烧');
	data=data.replace(/Spirit Bond/gi, '精神连结');
	data=data.replace(/Ether Renewal/gi, '魔力复原');
	data=data.replace(/Infuse Health/gi, '生之礼赞');
	data=data.replace(/Protective Bond/gi, '保护连结');
	data=data.replace(/Ebon Battle Standard of Honor/gi, '黑檀战斗荣誉规范');
	data=data.replace(/Whirling Defense/gi, '旋风防御');
	data=data.replace(/Dwarven Stability/gi, '矮人坚定');
