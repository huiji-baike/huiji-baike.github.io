...
$.ajax({
    url: 'http://wiki.guildwars.com/wiki/Zaishen_Challenge_Quest'+urlSuffix,
    type: 'GET',
...
$.ajax({
    url: 'http://wiki.guildwars.com/wiki/Daily_activities'+urlSuffix,
    type: 'GET',
    success: function(res) {
...
function prepShortData(data){
	...
	data=data.replace(/<a(.|\n){0,3}href="\/wiki\/Daily_activities" title="Daily activities">(.|n)*?<\/a>/gi, '<span style="padding: 0  5em;font-weight:700;color:#428bca;cursor: pointer;" onclick="kaiguan()"> 击此展开全表  (含光刃，先锋队，及[毁灭前]旅者) <\/span>'); //改说明

	//擦表格后各字, 即 The listed rewards are those 之前的html
	data=data.replace(/(^|[^A-Za-z])<dl>(.|\n)*?<dd>(.|\n)*?<small>(.|\n)*?"(?=[^A-Za-z]|$)/gi, '$1');

	data=data.replace(/(^|[^A-Za-z])(<th>(.|\n){0,3}Single reward(.|\n)*?<\/th>)(?=[^A-Za-z]|$)/gi, '$1<th>奖赏统计 ( <img alt="铜币" height="19" src="https://shoutwikis.github.io/战承文件/铜币.png" srcset="https://shoutwikis.github.io/战承文件/铜币29.png 1.5x, https://shoutwikis.github.io/战承文件/铜币38.png 2x" width="19"><a href="http://wiki.guildwars.com/wiki/Copper_Zaishen_Coin" title="铜战承币">铜战承币</a> )</th>');
...
function parseTranslate(data){
    ...
    data=data.replace(/(^|[^A-Za-z])(>Echovald ForestS*?<)(?=[^A-Za-z]|$)/gi, '$1>科凡德森林<');
	...
	//黑檀先锋队任务
	data=data.replace(/(^|[^A-Za-z])(>Countess Nadya<)(?=[^A-Za-z]|$)/gi, '$1>伯爵夫人 娜佳<');
	data=data.replace(/(^|[^A-Za-z])(>Footman Tate<)(?=[^A-Za-z]|$)/gi, '$1>仆从 泰特<');
	data=data.replace(/(^|[^A-Za-z])(>Bandits<)(?=[^A-Za-z]|$)/gi, '$1>草寇<');
	data=data.replace(/(^|[^A-Za-z])(>Utini Wupwup<)(?=[^A-Za-z]|$)/gi, '$1>尤提尼 乌坡乌坡<');
	data=data.replace(/(^|[^A-Za-z])(>Ascalonian Noble<)(?=[^A-Za-z]|$)/gi, '$1>阿斯卡隆 贵族<');
	data=data.replace(/(^|[^A-Za-z])(>Undead<)(?=[^A-Za-z]|$)/gi, '$1>不死族<');
	data=data.replace(/(^|[^A-Za-z])(>Blazefiend Griefblade<)(?=[^A-Za-z]|$)/gi, '$1>火焰恶魔 悲伤刀片<');
	data=data.replace(/(^|[^A-Za-z])(>Farmer Hamnet<)(?=[^A-Za-z]|$)/gi, '$1>农民 哈姆奈特<');
	...
	data=data.replace(/(^|[^A-Za-z])(>Forgewight<)(?=[^A-Za-z]|$)/gi, '$1>炼冶维特<') ;
	data=data.replace(/(^|[^A-Za-z])(>Baubao Wavewrath<)(?=[^A-Za-z]|$)/gi, '$1>怒浪 保博<');
	data=data.replace(/(^|[^A-Za-z])(>Joffs the Mitigator<)(?=[^A-Za-z]|$)/gi, '$1>缓和者 卓夫<');
	data=data.replace(/(^|[^A-Za-z])(>Rragar Maneater<)(?=[^A-Za-z]|$)/gi, '$1>拉喀 食人者<');
	data=data.replace(/(^|[^A-Za-z])(>Chung, the Attuned<)(?=[^A-Za-z]|$)/gi, '$1>得道者 村<');
	data=data.replace(/(^|[^A-Za-z])(>Lord Jadoth<)(?=[^A-Za-z]|$)/gi, '$1>霸王 贾多司<');
	data=data.replace(/(^|[^A-Za-z])(>Nulfastu, Earthbound<)(?=[^A-Za-z]|$)/gi, '$1>地缚 纳法斯图<');
	data=data.replace(/(^|[^A-Za-z])(>(?:The )?Iron Forgeman<)(?=[^A-Za-z]|$)/gi, '$1>钢铁巨人<');
	data=data.replace(/(^|[^A-Za-z])(>Magmus<)(?=[^A-Za-z]|$)/gi, '$1>麦格默斯<');
	...
	data=data.replace(/(^|[^A-Za-z])(>Quansong Spiritspeak<)(?=[^A-Za-z]|$)/gi, '$1>通灵者 魁嵩<');
	data=data.replace(/(^|[^A-Za-z])(>The Stygian Underlords<)(?=[^A-Za-z]|$)/gi, '$1>冥狱地王<');
	data=data.replace(/(^|[^A-Za-z])(>Fozzy Yeoryios<)(?=[^A-Za-z]|$)/gi, '$1>冻 耀尔伊欧<');
	data=data.replace(/(^|[^A-Za-z])(>The Black Beast of Arrgh<)(?=[^A-Za-z]|$)/gi, '$1>黑色野兽 阿尔古<');
	data=data.replace(/(^|[^A-Za-z])(>Arachni<)(?=[^A-Za-z]|$)/gi, '$1>奥拉赫妮<');
	data=data.replace(/(^|[^A-Za-z])(>The Four Horsemen<)(?=[^A-Za-z]|$)/gi, '$1>多姆四骑士<');
	data=data.replace(/(^|[^A-Za-z])(>Remnant of Antiquities<)(?=[^A-Za-z]|$)/gi, '$1>残余的断片<');
	data=data.replace(/(^|[^A-Za-z])(>Arbor Earthcall<)(?=[^A-Za-z]|$)/gi, '$1>大地之唤 亚伯<');
	data=data.replace(/(^|[^A-Za-z])(>Prismatic Ooze<)(?=[^A-Za-z]|$)/gi, '$1>多彩乌兹<');
	data=data.replace(/(^|[^A-Za-z])(>Lord Khobay<)(?=[^A-Za-z]|$)/gi, '$1>领主库贝<');
	data=data.replace(/(^|[^A-Za-z])(>Jedeh the Mighty<)(?=[^A-Za-z]|$)/gi, '$1>强者 捷地<');
	data=data.replace(/(^|[^A-Za-z])(>Ssuns, Blessed of Dwayna<)(?=[^A-Za-z]|$)/gi, '$1>薇娜的祝福 桑斯<');
	data=data.replace(/(^|[^A-Za-z])(>Justiciar Thommis<)(?=[^A-Za-z]|$)/gi, '$1>司法官 汤米兹<');
	data=data.replace(/(^|[^A-Za-z])(>Pywatt the Swift<)(?=[^A-Za-z]|$)/gi, '$1>迅捷 派维特<');
	data=data.replace(/(^|[^A-Za-z])(>Harn and Maxine Coldstone<)(?=[^A-Za-z]|$)/gi, '$1>冰石哈恩/麦辛<');
	data=data.replace(/(^|[^A-Za-z])(>Fendi Nin<)(?=[^A-Za-z]|$)/gi, '$1>梵帝宁<');
	data=data.replace(/(^|[^A-Za-z])(>Mungri Magicbox<)(?=[^A-Za-z]|$)/gi, '$1>魔法盒 牧格里<');
	data=data.replace(/(^|[^A-Za-z])(>Priest of Menzies<)(?=[^A-Za-z]|$)/gi, '$1>曼席斯的祭司<');
	data=data.replace(/(^|[^A-Za-z])(>Ilsundur, Lord of Fire<)(?=[^A-Za-z]|$)/gi, '$1>火之主 音丧多<');
	data=data.replace(/(^|[^A-Za-z])(>Kepkhet Marrowfeast<)(?=[^A-Za-z]|$)/gi, '$1>柯博海特 食髓者<');
	data=data.replace(/(^|[^A-Za-z])(>Commander Wahli<)(?=[^A-Za-z]|$)/gi, '$1>指挥官 瓦里<');
	data=data.replace(/(^|[^A-Za-z])(>Kanaxai<)(?=[^A-Za-z]|$)/gi, '$1>加奈赛<');
	data=data.replace(/(^|[^A-Za-z])(>Khabuus<)(?=[^A-Za-z]|$)/gi, '$1>卡布斯<');
	data=data.replace(/(^|[^A-Za-z])(>Molotov Rocktail<)(?=[^A-Za-z]|$)/gi, '$1>石尾 摩洛托夫<');
	data=data.replace(/(^|[^A-Za-z])(>The Stygian Lords<)(?=[^A-Za-z]|$)/gi, '$1>烈狱领主<');
	data=data.replace(/(^|[^A-Za-z])(>Dragon Lich<)(?=[^A-Za-z]|$)/gi, '$1>巫妖之龙<');

	//处理粗体字
	//console.log(data);
	data=data.replace(/<tr style="font-weight: bold;">(?=[^A-Za-z]|$)/gi, '<tr>');
	//console.log(data);
	//data=data.replace(/<tr>/gi, '<tr style="font-weight: bold;">');
	//console.log(data);
	var tNow = new Date();
	...
	//加文字日期
	var tSNow = tDateNow + ' ' + monthConversion(tMonthNow) + ' ' + tYearNow;
	var tSYesterday = tDateYesterday + ' ' + monthConversion(tMonthYesterday) + ' ' + tYearYesterday;
	...
	var tCutoff = new Date(tString1);
	var tCutoffSandford = new Date(tString2);
	console.log(tNow);
	console.log(tString1);
	console.log(tString2);
	console.log(tSNow);
	console.log(tCutoff);
	console.log(tCutoffSandford);
	console.log(tNow > tCutoff);

	if (tNow > tCutoff) {
	    var regExString = "<tr>(<td([^0-9]|\\n){0,50}"+tSNow+"(.|\\n)*?)<\\/tr>";
		console.log(regExString);
		var reg_Ex = new RegExp(regExString,"gi");
		data=data.replace(reg_Ex, fReplacer);
	}
	else{
		//use one date previous in the table, below uses Yesterday
		//var regExString = "<tr(>(.|\\n){0,1250}<tr><td(.|\\n){0,50}"+tSNow+")";
		var regExString = "<tr>(<td([^0-9]|\\n){0,50}"+tSYesterday+"(.|\\n)*?)<\\/tr>";
		console.log(regExString);
		var reg_Ex = new RegExp(regExString,"gi");
		...
		if (tNow > tCutoffSandford) {
		    //in terms of UTC
			//quit using yesterday's,
			regExString = "(<tr(.|\n){0,50}><td([^0-9]|\\n){0,50}"+tSYesterday+"(.|\\n)*?<\\/tr>)";
			reg_Ex = new RegExp(regExString,"gi")
			data=data.replace(reg_Ex, fReplacerEY);
			//and use today's:
			regExString = "(<tr(.|\n){0,50}><td([^0-9]|\\n){0,50}"+tSNow+"(.|\\n)*?<\\/tr>)";
			reg_Ex = new RegExp(regExString,"gi")
			data=data.replace(reg_Ex, fReplacerAT);
		}
...
function fReplacerEY(match, p1, p2, p3, offset, string) {
  if ((p1.match(/<a/gi)||[]).length < 7){
	console.log("returned match");
	return match;
  }
  else{
	//p1 should be the entire match containing <tr> and </tr>
	console.log("process match");
	var regExString = '<a style="color: '+highlight_Color+';"((.|\\n){0,110}<\\/tr>)';
	var reg_Ex = new RegExp(regExString,"gi")
	var ret = p1.replace(reg_Ex, '<a style="font-weight: normal;" $1');
	console.log(ret);
	return ret;
  }
}

function fReplacerAT(match, p1, p2, p3, offset, string) {
  if ((p1.match(/<a/gi)||[]).length < 7){
	console.log("returned match");
	return match;
  }
	...
	var regExString = '<a((.|\\n){0,110}<\\/tr>)';
	var reg_Ex = new RegExp(regExString,"gi")
	var ret = p1.replace(reg_Ex, '<a style="font-weight: bold;color: '+highlight_Color+';" $1');
	console.log(ret);
	return ret;
  }
}
